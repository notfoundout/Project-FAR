...
from .evidence_store import EvidenceStore, EvidenceStoreError, record_payload
...
class SecuredRuntime:
    ...
    def authorize(self, principal: Principal, request: dict[str, Any]) -> dict[str, Any]:
        ...
        policy_principal = Principal(
            principal.tenant_id,
            principal.key_id,
            tuple(sorted(set(principal.scopes) | {"policy:read"})),
        )
        ...
        manifest["files"]["authorization.json"] = hashlib.sha256(
            authorization_path.read_bytes()
        ).hexdigest()
        _write_json(manifest_path, manifest)
        record = self.evidence.put_directory(secured_directory)

        response = dict(result.payload)
        response.update(
            {
                "schema_version": SECURED_SERVICE_SCHEMA_VERSION,
                "tenant_id": principal.tenant_id,
                "evidence_id": secured_id,
                "policy": {
                    "policy_id": policy.policy_id,
                ...
                "evidence": {
                    "record": f"/v1/evidence/{secured_id}",
                    "manifest": f"/v1/evidence/{secured_id}/manifest",
                },
            }
        )
        return response
    ...
    def evidence_record(self, principal: Principal, evidence_id: str) -> dict[str, Any]:
        ...
        expected_prefix = f"{principal.tenant_id}-"
    ...
    def evidence_manifest(self, principal: Principal, evidence_id: str) -> dict[str, Any]:
        ...
        path = self.evidence.blob_root / evidence_id / "manifest.json"
        try:
    ...
    class SecuredAuthorizationHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if parsed.path == "/healthz":
                self._send(HTTPStatus.OK, {"status": "ok", "schema_version": SECURED_SERVICE_SCHEMA_VERSION})
                return
            try:
                if parsed.path.startswith("/v1/evidence/"):
                    ...
                    if suffix.endswith("/manifest"):
                        evidence_id = suffix.removesuffix("/manifest")
                        payload = runtime.evidence_manifest(principal, evidence_id)
                    else:
                        payload = runtime.evidence_record(principal, suffix)
                ...
                if parsed.path.startswith("/v1/policies/"):
                    ...
                    self._send(HTTPStatus.OK, runtime.policy(principal, policy_id, version))
                    return
            except SecurityError as exc:
                self._send(HTTPStatus.FORBIDDEN, {"error": "forbidden", "detail": str(exc)})
                return
            except EvidenceStoreError as exc:
                self._send(HTTPStatus.NOT_FOUND, {"error": "not_found", "detail": str(exc)})
                return
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                payload = runtime.authorize(principal, request)
            except SecurityError as exc:
                self._send(HTTPStatus.FORBIDDEN, {"error": "forbidden", "detail": str(exc)})
                return
