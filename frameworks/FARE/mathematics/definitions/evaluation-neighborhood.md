...
A **neighborhood system** `N(E)` is a collection of admissible neighborhoods of `E`.

An **evaluation neighborhood** of `E` is an element `U` of `N(E)`.

Every neighborhood `U` in `N(E)` must contain `E`.

---

# Distance Neighborhood

Given a non-negative radius `r`, the **distance neighborhood** `N_r(E)` is the collection `{X : d(E, X) <= r}`.

Distance neighborhoods form one possible neighborhood system.

Alternative neighborhood systems must satisfy explicitly stated admissibility conditions, including the requirement that every neighborhood of `E` contain `E`.
...
Future investigations may establish additional admissible neighborhood systems independent of distance.
