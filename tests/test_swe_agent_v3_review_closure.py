...
class ReviewClosureTests(unittest.TestCase):
    ...
    def test_complete_task_identity_contracts_are_exact_locked(self) -> None:
    ...
    def test_seed_derivation_recomputes_from_locked_preoutcome_inputs(self) -> None:
        self.assertEqual(
            module.hashlib.sha256(module.SEED_PAYLOAD.encode("utf-8")).hexdigest(),
            module.SEED_HEX,
        )
        self.assertEqual(len(bytes.fromhex(module.SEED_HEX)), 32)
