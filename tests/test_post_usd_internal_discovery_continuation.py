...
class PostUSDInternalDiscoveryContinuationTests(unittest.TestCase):
    ...
    def test_execution_order_is_complete_and_contiguous(self):
        ...
        self.assertEqual(program['registration_pr'],260); self.assertEqual([x['sequence'] for x in streams],list(range(1,10))); self.assertEqual([x['target_pr'] for x in streams],list(range(261,270))); self.assertEqual(streams[-1]['id'],'IKD-W9-TERMINAL-ADJUDICATION')
    ...
    def test_next_action_is_candidate_architecture_freeze(self):
        queue=self.load(QUEUE); self.assertEqual(queue['registration_pr'],260); next_pr=queue['next_action']['target_pr']; self.assertIn(next_pr,{261,262,263,264})
        ...
        if next_pr==264:self.assertEqual([x['target_pr'] for x in queue['completed_workstreams']],[261,262,263])
