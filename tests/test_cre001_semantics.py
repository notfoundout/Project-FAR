...
class CRE001SemanticRegressionTests(unittest.TestCase):
    ...
    def test_generated_task_ids_and_provenance_are_unambiguous(self):
        ...
        self.assertIn('### STRATEGIC-001: Execute the W4 S_core formal negative controls',ranked)
        self.assertIn('### STRATEGIC-002: Execute W3.5 baseline factorization and universal-discovery gate',ranked)
        self.assertIn('### STRATEGIC-003: Assemble the finite-core theorem or strongest obstruction',ranked)
        self.assertIn('The RCS-001 admission framework is frozen',ranked)
        self.assertIn('its concrete corpus is not',ranked)
