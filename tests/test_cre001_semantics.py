...
class CRE001SemanticRegressionTests(unittest.TestCase):
    ...
    def test_generated_task_ids_and_provenance_are_unambiguous(self):
        ...
        self.assertIn('### STRATEGIC-003: Execute candidate ablation and reconstruction',ranked)
        ...
        self.assertIn('registered reasoning discrimination, and the qualified FARA-specificity result are complete',ranked)
        self.assertIn('Candidate ablation and reconstruction are next',ranked)
        self.assertIn('Unique FARA discriminative capacity is refuted at registered scope',ranked)
