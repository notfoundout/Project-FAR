...
class CRE001SemanticRegressionTests(unittest.TestCase):
    ...
    def test_generated_task_ids_and_provenance_are_unambiguous(self):
        ...
        self.assertIn('### STRATEGIC-001: Build the S_core construction and obstruction ledger', ranked)
        self.assertIn('### STRATEGIC-002: Prove faithful-representation negative-control lemmas', ranked)
        self.assertIn('### STRATEGIC-003: Develop the first uniform S_core constructor', ranked)
        self.assertIn('THM-TARGET-001, FAITHFUL-REP-001, and P8-DEC-001 are frozen', ranked)
