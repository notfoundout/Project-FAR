...
class RepresentationDiscoverySeparationTests(unittest.TestCase):
    ...
    def test_w35_registers_only_the_corpus_artifact_as_complete(self):
        ...
        for a in self.w35['required_result_artifacts']:
            self.assertEqual(a['status'],'complete'); self.assertRegex(a['content_sha256'],r'^[0-9a-f]{64}$'); self.assertTrue((ROOT/a['path']).is_file()); self.assertTrue(a['artifact_id'])
    def test_candidates_are_not_prejudged(self):
        allowed={'supported_at_registered_scope','refuted_at_registered_scope','partial'}
    ...
    def test_w35_registers_immutable_evidence_fields(self):
        ...
        for a in by.values(): self.assertIn('path',a); self.assertIn('artifact_id',a); self.assertIn('content_sha256',a); self.assertEqual(a['status'],'complete'); self.assertRegex(a['content_sha256'],r'^[0-9a-f]{64}$')
    def test_w4_completion_does_not_resolve_w35(self):
        by_id={item['id']:item for item in self.ledger['obligations']}; self.assertEqual(by_id['OBS-SC-010']['status'],'obstruction_established'); self.assertEqual(self.w35['status'],'resolved'); self.assertTrue(self.w35['w5_authorized']); self.assertEqual(self.w35['current_results']['fara_specificity'],'not_unique_at_registered_scope'); self.assertEqual(self.w35['current_results']['candidate_invariants'],'complete')
