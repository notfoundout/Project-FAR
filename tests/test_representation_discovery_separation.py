...
class RepresentationDiscoverySeparationTests(unittest.TestCase):
    ...
    def test_w35_registers_only_the_corpus_artifact_as_complete(self):
        ...
        self.assertEqual(by_id['W35-CORPUS-RESULT']['artifact_id'],'RCS-CORPUS-001'); self.assertEqual(by_id['W35-FACTOR-RESULT']['artifact_id'],'W35-FACTOR-RESULT-001'); self.assertEqual(by_id['W35-SCOPE-RESULT']['artifact_id'],'W35-SCOPE-RESULT-001'); self.assertEqual(by_id['W35-SPEC-RESULT']['artifact_id'],'W35-SPEC-RESULT-001'); self.assertEqual(by_id['W35-CANDIDATE-RESULT']['artifact_id'],'W35-CANDIDATE-RESULT-001')
    ...
    def test_candidates_are_not_prejudged(self):
        self.assertTrue(all(item['current_classification']!='unresolved' for item in self.candidates['candidates'])); self.assertTrue(all(item['current_classification']!='indispensable_within_frozen_class' for item in self.candidates['candidates'])); self.assertEqual(self.candidates['aggregate_result'],'no_registered_candidate_indispensable_within_frozen_class'); self.assertEqual(self.candidate_result['claim_effect']['universal_structure'],'unresolved')
    ...
    def test_status_only_authorization_is_rejected(self):
        w35=copy.deepcopy(self.w35); target=copy.deepcopy(self.rep_target); w35['w5_authorized']=True; w35['status']='resolved'; target['w5_authorization']['authorized']=True; target['w5_authorization']['blocked_by']=[]; errors=authorization_errors(w35,target,self.scope,self.gates,self.ledger,ROOT); self.assertTrue(errors); self.assertFalse(any('candidate_invariants must be complete' in e for e in errors)); self.assertTrue(any('machinery_and_cost must be complete' in e for e in errors)); self.assertTrue(any('is not complete' in e for e in errors))
    def test_w35_registers_immutable_evidence_fields(self):
        ...
        for aid in ('W35-CORPUS-RESULT','W35-FACTOR-RESULT','W35-SCOPE-RESULT','W35-SPEC-RESULT','W35-CANDIDATE-RESULT'): self.assertEqual(by[aid]['status'],'complete'); self.assertRegex(by[aid]['content_sha256'],r'^[0-9a-f]{64}$')
    def test_w4_completion_does_not_resolve_w35(self):
        by_id={item['id']:item for item in self.ledger['obligations']}; self.assertEqual(by_id['OBS-SC-010']['status'],'obstruction_established'); self.assertEqual(self.w35['status'],'in_progress_candidate_complete'); self.assertFalse(self.w35['w5_authorized']); self.assertEqual(self.w35['current_results']['fara_specificity'],'not_unique_at_registered_scope'); self.assertEqual(self.w35['current_results']['candidate_invariants'],'complete_no_indispensable_candidate')
