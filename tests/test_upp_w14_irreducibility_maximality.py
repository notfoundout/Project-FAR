...
class TestUPPW14(unittest.TestCase):
    ...
    def test_valid_reduction_refutes_irreducibility(self):
        ledger = m.canonical_ledger()
        valid = m.ReductionCandidate(
            "valid-reduction", frozenset(), frozenset(m.PRESERVATION_DIMENSIONS),
            frozenset(m.REDUCTION_REQUIREMENTS), True, True, True
        )
    ...
    def test_unresolved_reduction_keeps_unknown(self):
        ledger = m.canonical_ledger()
        unknown = m.ReductionCandidate(
            "unknown-reduction", frozenset(), frozenset(m.PRESERVATION_DIMENSIONS),
            frozenset(m.REDUCTION_REQUIREMENTS), True, True, True,
            unresolved_facts=("equivalence_unresolved",)
        )
    ...
    def test_hidden_machinery_reduction_fails(self):
        c = m.ReductionCandidate(
            "hidden", frozenset(), frozenset(m.PRESERVATION_DIMENSIONS),
            frozenset(m.REDUCTION_REQUIREMENTS) - {"machinery_closed"}, True, True, True
        )
        self.assertEqual(m.reduction_verdict(c), m.Verdict.REFUTED)

    def test_circular_reduction_fails(self):
        c = m.ReductionCandidate(
            "circular", frozenset(), frozenset(m.PRESERVATION_DIMENSIONS),
            frozenset(m.REDUCTION_REQUIREMENTS) - {"non_circular"}, True, True, True
        )
        self.assertEqual(m.reduction_verdict(c), m.Verdict.REFUTED)

    def test_information_loss_fails(self):
        c = m.ReductionCandidate(
            "lossy", frozenset(), frozenset(m.PRESERVATION_DIMENSIONS) - {"information"},
            frozenset(m.REDUCTION_REQUIREMENTS), True, True, True
        )
        self.assertEqual(m.reduction_verdict(c), m.Verdict.REFUTED)
    ...
    def test_cost_shift_fails(self):
        c = m.ReductionCandidate(
            "cost-shift", frozenset(), frozenset(m.PRESERVATION_DIMENSIONS),
            frozenset(m.REDUCTION_REQUIREMENTS), True, True, False
        )
        self.assertEqual(m.reduction_verdict(c), m.Verdict.REFUTED)

    def test_in_class_nonembedding_refutes_maximality(self):
        ledger = m.canonical_ledger()
        bad = m.ExtensionChallenge(
            "novel", frozenset(m.MAXIMALITY_REQUIREMENTS), True, False,
            frozenset(m.PRESERVATION_DIMENSIONS)
        )
    ...
    def test_unresolved_extension_keeps_unknown(self):
        ledger = m.canonical_ledger()
        u = m.ExtensionChallenge(
            "unresolved", frozenset(m.MAXIMALITY_REQUIREMENTS), None, None,
            frozenset(), unresolved_facts=("membership_unresolved",)
        )
    ...
    def test_out_of_class_challenge_is_not_counterexample(self):
        c = m.ExtensionChallenge(
            "outside", frozenset(m.MAXIMALITY_REQUIREMENTS), False, False, frozenset()
        )
