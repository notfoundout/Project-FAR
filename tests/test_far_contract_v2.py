...
class FARContractV2RootOfTrustRegressionTests(unittest.TestCase):
    """Defects found by the 2026-09 root-of-trust audit.

    Expected sequences are derived by hand from specification sections 3.3, 5 and 9 and from
    RFC 8259, not recorded from the verifier. Before the repair, a repeated class id merged two
    declared classes, so `P1` below was certified as the exact observational quotient.
    """
    ...
    def test_repeated_class_id_is_rejected_even_on_the_exact_partition(self) -> None:
        ...
        self.assertEqual(self.codes(document), ["DUPLICATE_QUOTIENT_CLASS"])
    ...
    def test_overlapping_classes_are_not_a_partition(self) -> None:
        ...
        self.assertEqual(self.codes(document), ["QUOTIENT_OVERLAP", "QUOTIENT_NOT_PARTITION"])
    ...
    def test_frozen_record_requires_rfc3339_freeze_time(self) -> None:
        for frozen_at in ("2026-08-29 20:00:00Z", "2026-02-30T00:00:00Z", "2026-08-29T20:00:00", "yesterday"):
            with self.subTest(frozen_at=frozen_at):
                ...
                self.assertEqual(self.codes(document), ["FREEZE_TIME_INVALID"])

    def test_intake_rejects_non_json_constants_and_duplicate_keys(self) -> None:
        import tempfile

        from mechanization.far_mechanization.contract_v2 import load_and_validate
        ...
        for name, variant in variants.items():
            ...
            with self.subTest(variant=name), tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual([d.code for d in load_and_validate(path).diagnostics], ["UNREADABLE_CONTRACT"])
