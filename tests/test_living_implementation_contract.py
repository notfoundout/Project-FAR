...
IMPLEMENTATION_POLICY = {
    ...
    "authorization_registry": "research/living/implementation-authorizations-v1.0.json",
    ...
    "assurance_sensitive_exact_paths": [
        ".github/workflows/validator-assurance.yml",
        "requirements.txt",
        "tests/test_validation_assurance.py",
        "tools/assemble_validation_certificate.py",
    ],
}
...
def fixture(required: bool = True, target: str = TARGET):
    ...
    canonical.mkdir()
    source.mkdir()
    put_json(canonical, "research/living/implementation-policy-v1.0.json", IMPLEMENTATION_POLICY)
    ...
    review = {
        "candidate_id": CID,
        "source_key": "doi:10.0000/test",
        "disposition": "PROJECT_CHANGE_REQUIRED",
        "reviewed_on": "2026-09-13",
        "review_basis": "docs/audits/test.md",
        "proposal_id": "FAR-LIVING-PROP-TEST1",
    ...
    if required:
        auths.append({
            "proposal_id": PID,
            "candidate_id": CID,
            "proposal_sha256": sha256(proposal_raw),
            "candidate_sha256": sha256(candidate),
            "operations_sha256": canonical_json_sha([operation]),
            "review_record_sha256": canonical_json_sha(review),
            "authorization_status": "ACCEPTED_FOR_PROTECTED_IMPLEMENTATION_PR",
        })
    put_json(canonical, "research/living/implementation-authorizations-v1.0.json", {
        "schema_version": "1.0",
        "program_id": "FAR-LIVING-IMPLEMENTATION-AUTHORIZATIONS-001",
        "authority": "Research",
...
class LivingImplementationContractTests(unittest.TestCase):
    ...
    def test_policy_declaration_drift_fails_closed(self):
        tmp, canonical, source, _ = fixture()
        try:
            drifted = dict(IMPLEMENTATION_POLICY)
            drifted["payload_prefix"] = "research/living/inbox/other-payloads/"
            put_json(canonical, "research/living/implementation-policy-v1.0.json", drifted)
            with self.assertRaisesRegex(ImplementationContractError, "implementation policy drift: payload_prefix"):
