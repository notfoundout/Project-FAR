...
class InvestigationExecutionGateTests(unittest.TestCase):
    ...
    def test_required_step_id_must_be_non_empty_string(self):
        for invalid_id in (None, 7, "  "):
            with self.subTest(invalid_id=invalid_id):
                payload = self.passing_payload()
                payload["required_steps"][0]["id"] = invalid_id
                self.assertTrue(
                    any("missing, empty, or non-string id" in error for error in self.validate(payload))
                )
