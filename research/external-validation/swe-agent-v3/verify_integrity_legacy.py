...
def _decode_json(data: bytes, label: str) -> Any:
    ...
    try:
        return json.loads(data.decode("utf-8"), parse_constant=_bad_constant, object_pairs_hook=_unique_object)
...
def _committed_blob_bytes(relative: str) -> bytes:
    try:
        return subprocess.run(["git", "-C", str(ROOT), "cat-file", "blob", f"HEAD:{relative}"], check=True, capture_output=True).stdout
...
def _safe_path(relative: str) -> Path:
    ...
    if p.is_absolute() or not p.parts or any(part in {"", ".", ".."} for part in p.parts) or "\\" in relative:
...
def verify_manifest() -> dict[str, dict[str, Any]]:
    ...
    if manifest.get("scope") != "design-only authority; exact-locks all governed data, narrative, prospective-amendment, seed-commitment, and immutable historical-authority snapshot artifacts while verifier code remains reviewable and non-self-referential":
    ...
    for entry in entries:
        ...
        relative, blob, size = entry["path"], entry["git_blob_sha1"], entry["bytes"]
        ...
        seen.add(relative); index[relative] = entry
...
def verify_byte_policy() -> None:
    expected = b"# Preserve exact governed bytes regardless of core.autocrlf or platform defaults.\n* -text\n"
