...
This package freezes the causal question, arm structure, estimands, outcome contract, evidence requirements, capsule constraints, exact bootstrap procedure and bootstrap seed, task-manifest and sealed-identity-ledger contracts, required task strata, critical-harm threshold contract, and fail-closed execution gate. It does not select the confirmatory tasks, instantiate the sealed identity ledger, choose a model endpoint, build the FAR capsule, build the placebo, set the final run budget, freeze assignment/counterbalancing randomization, or authorize a pilot or confirmatory run.
...
The future frozen task manifest contains blinded task/repository labels, a unique task-bundle root, and prospective strata. A separate sealed identity ledger governed by `task-manifest-contract-v1.0.json` must bind every manifest position one-to-one to the authoritative GitHub repository ID, canonical audit URL, exact repository commit, task-payload digest/byte count, and matching strata. Before any pilot or confirmatory execution, an independent identity auditor must recompute every task root, enforce the minimum 24 tasks, at least five provider-stable repositories, the 20% per-repository cap, one-to-one blind repository mapping, and coverage of all five required strata. The exact task-manifest and sealed-ledger bytes and Git blob identities must be committed before execution; the ledger remains sealed from the executing agent and capsule authors.
...
A true gate alone can never authorize a run. Every pilot or confirmatory launch also requires a separately committed prospective launch record containing every identity in `launch_record_required_bindings`, including the frozen design commit/manifest, task manifest, sealed identity ledger, seed commitment, harm contract, treatment/placebo roots, model, prompts/configuration, environment/dependencies, budgets/stopping rules, assignment seed, grader, evidence-store configuration, protection reference, and manual authorization record.

`historical-authority-v1.0.json` roots the two exact historical snapshots needed to interpret the v1.1 failure/arithmetic amendment. Those snapshots are immutable historical evidence, not current design authority, and verification does not require repository history beyond the reviewed checkout. For unrelated current subjects, authority comes from the current prospectively amended, integrity-rooted design artifacts rather than from the historical amendment.
