...
The machine-readable file contains exactly `status`, `presentation_isolation`, and `schedules`. `status` is `FROZEN`. `presentation_isolation` contains exactly four Boolean flags, all `true`: `fresh_context`, `condition_label_hidden`, `same_case_alternate_packet_hidden`, and `other_evaluator_scores_hidden`. `schedules` contains exactly one object for each eligible evaluator and no others. Each schedule object contains exactly `evaluator_id`, the evaluator's frozen `lane`, and `assignments`. Each assignment contains exactly `presentation_index`, one-based `round`, `case_id`, `condition`, and the exact `run_id` for that case-condition pair in `execution-schedule.json`.
