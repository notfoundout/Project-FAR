...
| `scripts/compare_verifiers.py` | Runs the repository verifiers against both corpora and, optionally, a random corpus. | Reads the oracle expectations; it does not generate them. |
...
`compare_verifiers.py` reports differences on exactly two `far-ir/2.0` records. Both are deliberate rule changes of the v1.2 specification:
...
For `far-ir/2.1` it requires identical verdicts and diagnostic multisets. The 1211 order-only differences on the random corpus come from the oracle's grouped reading of the metric-axiom order. The v1.1 text allowed that reading. The v1.2 text now states the reference implementation's interleaved order, which was checked against 1218 records with metric-axiom failures.
