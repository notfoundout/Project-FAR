...
def validate_empirical_authority(
    ...
    if not isinstance(execution_gate, dict) or execution_gate.get("authorized") is not False:
        raise VerificationError("empirical execution gate must remain closed")
    blockers = execution_gate.get("blockers")
    if not isinstance(blockers, list) or not blockers or any(type(item) is not str or not item for item in blockers):
    ...
    if ledger.get(expected_charter_path) != EXPECTED_EMPIRICAL_BLOBS["scope-and-universality-charter-v1.1.md"]:
        raise VerificationError("empirical manifest no longer registers the controlling charter identity")
    if ledger.get(expected_audit_path) != EXPECTED_EMPIRICAL_BLOBS["chat-audit-2026-08-05.md"]:
        raise VerificationError("empirical manifest no longer registers the chat-audit identity")
