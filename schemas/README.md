...
`far-epistemic-v1.schema.json` defines the additive `far-epistemic/1.0` belief-to-learning interchange. It has separate probability and utility types and does not reinterpret `far-ir/2.1` approximation or cost semantics. Cross-record and arithmetic invariants are implemented by `mechanization.far_mechanization.epistemic`.
