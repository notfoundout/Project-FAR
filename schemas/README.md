...
The v1 schema remains used by its executable parser, normalizer, serialization, graph, CLI, and conformance suite. `far-ir/2.0` is intentionally separate: `mechanization.far_mechanization.contract_v2_strict_v11` is the current verifier for its schema and finite-explicit comparison evidence (it applies [errata 1](../docs/specification/far-ir-2.0-errata-1.md) to the frozen `contract_v2.py` baseline and rejects `PROVED`/`REFUTED` outcomes without checked evidence; see [current verification](../docs/specification/far-ir-2.0-current-verification.md)), `migrate_v1_to_v2` performs loss-explicit migration, and `contract_conformance` runs the registered v2 fixtures.
