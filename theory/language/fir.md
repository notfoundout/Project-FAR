...
A representation object records an explicit item participating in reasoning:

- `id`: local representation identifier.
- `kind`: role such as premise, observation, rule, conclusion, hypothesis, authority, likelihood, or counterexample.
- `content`: prose or symbolic content.
- `statement`: optional structured statement object when the content can be represented by `kind`, `subject`, `predicate`, `scope`, and `claim`.

## Transition objects

A transition object records movement from one reasoning state to another:

- `id`: transition identifier.
- `source`: source state label.
- `rule`: rule identifier.
- `target`: target state label.
- `status`: admissible, inadmissible, unresolved, or draft.
- `order`: integer ordering key for derivation-tree construction.

## Derivation objects

A derivation object is the ordered trace produced from transition objects. It records which rule fired, which state changed, and whether the transition is currently accepted. Derivations are machine-readable proof traces rather than replacements for canonical Markdown proofs.

## Reasoning-system objects

A reasoning-system object records how an external reasoning system is tested against FAR primitives:

- `system`: name of the external reasoning system.
- `far_primitives`: mappings for Investigation, Representation, Representational Structure, Interpretation, and Reasoning Calculus.
- `cleanly_maps`: aspects represented without additional assumptions.
- `requires_assumptions`: aspects represented only after explicit assumptions.
- `not_yet_represented`: gaps in the current encoding.
- `verdict`: draft, fits FAR, extends FAR, or falsifies FAR.

## External-system mapping objects

A mapping between an external reasoning system and FAR is successful only when the example identifies the reasoning activity, the representations involved, their organization, the interpretation assigning meaning, and the calculus governing transitions. If one of these cannot be supplied, the example becomes falsification evidence rather than a successful mapping.
