...
| Outcome asymmetry | A packet listing mostly positive outcomes signals that a positive result is wanted | **Repaired.** The "equally acceptable" list contains six outcomes, of which five are negative or dissolving, and the packet states outright that there is no target answer |
...
| Tier inflation | Context separation could be reported as independence | Constrained: five tiers with explicit `cannot_establish` lists; `T1`/`T2` may never be called replication; `LIM-031` addressed only from `T4`, closed only at `T5` |
