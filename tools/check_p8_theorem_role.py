#!/usr/bin/env python3
"""Validate P8-ROLE-001 after W1 internal direct-axis construction."""
...
LEDGER=ROOT/'theory/evaluation/s-core-construction-obstruction-ledger.json'; W1=ROOT/'theory/evaluation/s-core-w1-direct-axis-proof.json'; GATES=ROOT/'theory/evaluation/research-gates.json'
def load(p): return json.loads(p.read_text(encoding='utf-8'))
def main()->int:
    for p in (DOC,REG,TARGET,PREMISES,LEDGER,W1,GATES): assert p.is_file(),p
    ...
    assert reg['next_required_artifact']=='W2 dynamics history revision and self-modification proof-or-obstruction package'
    ...
    premises=load(PREMISES); assert premises['version']=='1.4'; assert next(x for x in premises['entries'] if x['id']=='PRM-010')['decision']=='split'
    ledger=load(LEDGER); by_id={x['id']:x for x in ledger['obligations']}; assert by_id['LEM-SC-014']['status']=='proved'; assert by_id['OBS-SC-006']['status']=='refuted'; assert ledger['execution_summary']['open']==23
    ...
    required=set(load(GATES)['required_canonical_artifacts']); assert W1.relative_to(ROOT).as_posix() in required
    gates={x['name']:x for x in load(GATES)['gates']}; assert gates['scoped-representation-proof']['status']=='not_satisfied'
    print('P8 theorem-role decision: PASS (P8-I direct embedding proved; recovery and Corr_8E unproved)')
    return 0
