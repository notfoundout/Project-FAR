...
RESULT=ROOT/'theory/evaluation/w3-5-factorization-result-v1.0.json'; WITNESSES=ROOT/'theory/evaluation/w3-5-factorization-witnesses-v1.0.json'; BASELINE=ROOT/'theory/evaluation/generic-relational-baseline-v1.0.json'; W35=ROOT/'theory/evaluation/w3-5-specificity-and-discovery-gate.json'; GATES=ROOT/'theory/evaluation/research-gates.json'; TARGET=ROOT/'theory/evaluation/thm-target-001.json'; CANDIDATE=ROOT/'theory/evaluation/w3-5-candidate-test-result-v1.0.json'
EXPECTED_DIMENSIONS={'expressiveness':'equivalent','translation':'bidirectional','constraint_strength':'fara_stricter','reasoning_specificity':'not_established','cost_relation':'tradeoff','overall_interpretation':'fara_constrained_equivalent'}
...
def validate_static(root:Path)->dict[str,Any]:
    paths={name:root/path.relative_to(ROOT) for name,path in {'result':RESULT,'witnesses':WITNESSES,'baseline':BASELINE,'w35':W35,'gates':GATES,'target':TARGET,'candidate':CANDIDATE}.items()}
    for path in paths.values():
        if not path.is_file(): raise ValidationError(f'missing factorization dependency: {path.relative_to(root)}')
    result,witnesses,baseline=load(paths['result']),load(paths['witnesses']),load(paths['baseline']); w35,gates,target,candidate=load(paths['w35']),load(paths['gates']),load(paths['target']),load(paths['candidate'])
    if result.get('artifact_id')!='W35-FACTOR-RESULT-001' or result.get('status')!='complete': raise ValidationError('factorization result artifact is not complete')
    if result.get('dimensions')!=EXPECTED_DIMENSIONS or result.get('scope',{}).get('instance_count')!=18: raise ValidationError('factorization dimensions or scope changed')
    ...
    records=witnesses.get('records'); ids=[item.get('instance_id') for item in records or []]
    if witnesses.get('status')!='frozen_expected_runtime_witnesses' or len(ids)!=18 or len(set(ids))!=18 or witnesses.get('class_counts')!={'positive':8,'contrast':8,'disputed':2}: raise ValidationError('factorization witness registry scope changed')
    ...
    for item in records:
        if item.get('status')!='pass_required' or set(item.get('required_checks',[]))!=required_checks: raise ValidationError(f"{item.get('instance_id')}: runtime obligations changed")
    if baseline.get('baseline_id')!='GREL-001' or baseline.get('current_result')!=EXPECTED_DIMENSIONS: raise ValidationError('GREL-001 result does not match factorization')
    ...
    if w35.get('status')=='in_progress_factorization_complete':
        for name in ('fara-specificity-resolved','reasoning-contrast-execution'):
            if gate_map.get(name,{}).get('status')!='not_satisfied': raise ValidationError(f'{name} was promoted without a later-stage result')
    else:
        for name in ('fara-specificity-resolved','reasoning-contrast-execution'):
...
def validate_runtime(root:Path,witnesses:dict[str,Any])->dict[str,Any]:
    ...
    if report.get('dimensions')!=EXPECTED_DIMENSIONS: raise ValidationError('runtime factorization dimensions changed')
    runtime={item['instance_id']:item for item in report.get('records',[])}; expected={item['instance_id']:item for item in witnesses.get('records',[])}
    if set(runtime)!=set(expected): raise ValidationError('runtime factorization instance set differs from frozen registry')
    for instance_id,item in runtime.items():
        if item.get('status')!='pass' or any(value!='pass' for value in item.get('checks',{}).values()): raise ValidationError(f'{instance_id}: runtime factorization failed')
        grel,fara=item.get('cost',{}).get('grel',{}),item.get('cost',{}).get('fara',{})
        if grel.get('schema_components')!=10 or fara.get('architecture_fields')!=14 or fara.get('witness_fields')!=5: raise ValidationError(f'{instance_id}: architecture surface changed')
        if grel.get('semantic_axis_access_operations',0)<=fara.get('semantic_axis_access_operations',0): raise ValidationError(f'{instance_id}: declared FARA semantic-access advantage disappeared')
    return report
def validate(root:Path=ROOT)->dict[str,Any]:
    static=validate_static(root); runtime=validate_runtime(root,static['witnesses']); return {'status':'pass','artifact_id':static['result']['artifact_id'],'runtime_experiment_id':runtime['experiment_id'],'instance_count':runtime['scope']['instances'],'dimensions':runtime['dimensions']}
def main(argv:list[str]|None=None)->int:
    parser=argparse.ArgumentParser(description='Validate W3.5 GREL-FARA factorization'); parser.add_argument('--root',type=Path,default=ROOT); parser.add_argument('--json',action='store_true'); args=parser.parse_args(argv)
    try: report=validate(args.root.resolve())
    except (ValidationError,FactorizationError,OSError,KeyError,TypeError,ValueError,json.JSONDecodeError) as exc: print(f'FAR-VAL-FACTOR-001: {exc}'); return 1
    if args.json: print(json.dumps(report,indent=2,sort_keys=True))
    else: print(f"W3.5 factorization validation: PASS ({report['instance_count']} instances; {report['dimensions']['overall_interpretation']})")
    return 0
