...
def main()->int:
    ...
    if (ROOT/'artifacts/governance/live-branch-pr-snapshot-v1.0.json').exists():
    ...
    if failures:
        ...
        for name,cmd,code,output,timed_out in failures:
            print(f'- name: {name}'); print(f'  command: {" ".join(cmd)}'); print(f'  exit code: {code}'); print(f'  kind: {"timeout" if timed_out else "failure"}'); print('  last 30 output lines:')
