...
CORE_THEORY_ID = "PROJECT-FAR-CORE-THEORY-1.1"
...
ARTIFACTS: tuple[Artifact, ...] = (
    ...
    Artifact(
        ROOT / "theory/theorems/Project-FAR-Theory-Closure-v1.0.md",
        Path("theorems/Project-FAR-Theory-Closure-v1.0.md"),
        "theorems",
        status="historical",
    ),
    Artifact(
        ROOT / "theory/theorems/Project-FAR-Theory-Closure-v1.1.md",
        Path("theorems/Project-FAR-Theory-Closure-v1.1.md"),
        "theorems",
    ),
    Artifact(
        ROOT / "theory/terminal/project-far-core-theory-v1.1.json",
        Path("theorems/project-far-core-theory-v1.1.json"),
        "theorems",
    ),
...
def write_generated(out: Path) -> list[dict]:
    ...
    compat.write_text(
        f"""# Project FAR Specification Compatibility Policy
...
def build(out: Path) -> None:
    ...
    for record in records:
        record["sha256"] = checksums[record["path"]]
    manifest = {
        **metadata,
...
def validate(out: Path) -> None:
    ...
    if missing:
        raise ValueError(f"missing manifest metadata: {missing}")
    if manifest["core_theory_id"] != CORE_THEORY_ID:
        raise ValueError("manifest core_theory_id does not match exporter authority")
    if not re.fullmatch(r"[0-9a-f]{40}", manifest["commit_sha"]):
        raise ValueError("manifest commit_sha is not a full Git SHA")
    paths = [a["path"] for a in manifest["artifacts"]]
    if len(paths) != len(set(paths)):
        raise ValueError("duplicate artifact names exist")
    ...
    if sorted(paths + ["manifest.json"]) != actual:
        raise ValueError("manifest does not match exported files")
    md_link = re.compile(r"\[[^\]]+\]\((?!https?://|mailto:|#)([^)]+)\)")
    for path in out.rglob("*.md"):
        for ref in md_link.findall(path.read_text(encoding="utf-8")):
            target = (path.parent / ref.split("#")[0]).resolve()
            if ref.split("#")[0] and out.resolve() in target.parents and not target.exists():
                raise ValueError(f"broken reference in {path}: {ref}")
