#!/usr/bin/env python3
"""Parse machine-readable FAR YAML files into FIR-style FARObject structures."""
...
def require_list(data: Dict[str, Any], key: str) -> Iterable[Any]:
    value = data.get(key, [])
    if value is None:
        return []
...
def load_far_yaml(path: Path) -> FARObject:
    ...
    if isinstance(data.get("reasoning_system"), dict):
        reasoning_system = ReasoningSystemMapping.from_mapping(data["reasoning_system"])

    far = FARObject(investigation=investigation, reasoning_system=reasoning_system)

    for item in require_list(data, "representations"):
        ...
        if "statement" in item:
            statement = Statement.from_value(item["statement"], f"representation {rep_id}")
        rep = Representation(
            id=rep_id,
    ...
    for item in require_list(data, "relations"):
        if not isinstance(item, dict):
    ...
    for item in require_list(data, "interpretations"):
        if not isinstance(item, dict):
    ...
    for item in require_list(data, "rules"):
        if not isinstance(item, dict):
    ...
    for item in require_list(data, "transitions"):
        if not isinstance(item, dict):
