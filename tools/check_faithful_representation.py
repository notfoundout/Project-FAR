...
def main()->int:
 ...
 w5=load(W5); assert w5['proof_id']=='SCORE-W5-PROOF-001'; assert w5['claim_effect']['bounded_faithful_representation']=='proved'; assert w5['claim_effect']['S_IRD_representation']=='unresolved'
 target=load(TARGET)
 if target['proof_status']=='bounded_theorem_proved':
  rep=next(x for x in target['theorem_family'] if x['id']=='THM-CORE-REP-001'); assert rep['status']=='proved_for_S_core' and rep['blocked_by']==[]
 else:
