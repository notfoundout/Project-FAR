...
def main()->int:
    ...
    require(candidate_result.get('artifact_id')=='W35-CANDIDATE-RESULT-001','candidate-result identity changed',errors); require(candidate_result.get('status')=='complete_project_authored_internal_execution','candidate result must be complete at the registered internal scope',errors); require(candidate_result.get('aggregate_result')=='registered_candidate_axes_resolved_at_frozen_internal_scope','candidate aggregate result changed',errors); require(candidate_result.get('claim_effect',{}).get('universal_structure')=='unresolved','candidate result promoted universal structure',errors); require(candidate_result.get('claim_effect',{}).get('W5_authorized') is False,'candidate result retrospectively authorized W5',errors)
    execution=candidate_result.get('execution',{}); require(execution.get('preserved_atomic_trials')==648,'candidate result must preserve 648 completed atomic trials',errors); require(execution.get('expected_atomic_trials')==648,'candidate execution must require exactly 648 atomic trials',errors); require(all(execution.get(k) is True for k in ('ablation_evidence_complete','reconstruction_evidence_complete','equivalent_reintroduction_evidence_complete','machinery_cost_complete')),'candidate execution evidence is incomplete',errors)
    results=candidate_result.get('results',[]); require(len(results)==12,'candidate result must preserve all twelve hypotheses',errors); require(all(item.get('trial_count')==54 and item.get('trial_evidence_status')=='complete' for item in results),'candidate trial coverage is incomplete',errors); require(all(all(item.get(axis) in REGISTERED_OUTCOMES for axis in AXES) for item in results),'candidate axis contains unresolved or invalid result',errors)
    ...
    require(w35.get('gate_id')=='W3.5-SDG-001','W3.5 gate id mismatch',errors); require(w35.get('status')=='resolved','W3.5 must be evidence-backed resolved',errors); require(w35.get('w5_authorized') is True,'W5 must be authorized after closure',errors)
    artifacts={a.get('id'):a for a in w35.get('required_result_artifacts',[])}; require(set(artifacts)=={'W35-CORPUS-RESULT','W35-FACTOR-RESULT','W35-SCOPE-RESULT','W35-SPEC-RESULT','W35-CANDIDATE-RESULT','W35-COST-RESULT','W35-CLAIM-RESULT','W35-FAILURE-RESULT'},'W3.5 artifact set changed',errors)
    for aid,a in artifacts.items(): require(a.get('status')=='complete',f'{aid} must be complete',errors)
    current=w35.get('current_results',{}); require(current.get('candidate_invariants')=='complete','candidate invariants must be complete',errors); require(current.get('machinery_and_cost')=='complete','cross-package machinery and cost closure must be complete',errors); require(current.get('claim_impact')=='complete_registered_scope_no_universal_promotion','claim impact closure is incomplete',errors); require(current.get('preserved_failures')=='complete','preserved failure closure is incomplete',errors)
    if len(w35.get('authorization_evidence',[])) != 8: errors.append('W3.5 authorization evidence must enumerate all eight result artifacts')
    ...
    if any(not a.get('path') for a in artifacts.values()): errors.append('a completed W3.5 artifact lacks a path')
    if target.get('w5_authorization',{}).get('authorized') and target.get('w5_authorization',{}).get('blocked_by'): errors.append('authorized W5 target retains blockers')
    if any(x.get('status')=='proved' for x in universal_target.get('theorem_families',[])): errors.append('W3.5 closure promoted a universal theorem')
...
def report(errors:list[str])->int:
    if errors:
        ...
        return 1
    print('Representation-discovery separation PASS (REP isolated; W3.5 resolved; W5 authorized; USD unresolved)')
    return 0
