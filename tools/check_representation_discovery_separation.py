...
def main()->int:
    ...
    baseline=load(PATHS['baseline']); scope=load(PATHS['scope']); corpus=load(PATHS['corpus_result']); us=load(PATHS['us_target'])
    ...
    require(baseline.get('status')=='frozen_prospective','generic baseline must be frozen prospectively',e)
    ...
    require(all(current.get(x)=='unresolved' for x in DIMS),'generic baseline results must remain unresolved before execution',e)
    ...
    require(w35.get('status')=='in_progress_corpus_frozen','W3.5 status must record corpus-frozen progress only',e)
    ...
    for aid,a in amap.items():
        if aid!='W35-CORPUS-RESULT': require(a.get('status')=='missing',f'{aid} must remain missing before execution',e)
...
def report(errors:list[str])->int:
    if errors:
        ...
        return 1
    print('Representation-discovery separation PASS (REP isolated; W4 terminal; corpus frozen; execution open; W3.5 evidence-blocks W5; USD unresolved)')
    return 0
