...
def generate_index():
    ...
    for cat,roots in cats.items():
        lines += [f'## {cat}','']; files=[]
        for r in roots: files += iter_index_files(ROOT/r)
...
def block():
    ...
    nav=[('Project Status',STATUS),('Research Gap Report',GAPS),('Next Actions',NEXT),('Dashboard Metrics',METRICS),('Deduction-First Standard','docs/governance/deduction-first-research-standard.md'),('Proof Roadmap','docs/planning/deduction-first-proof-roadmap.md'),('THM-TARGET-001','docs/research/thm-target-001-v1.0.md'),('Faithful Representation','docs/research/faithful-representation-specification-v1.0.md'),('Faithful Representation Registry','theory/evaluation/faithful-representation-specification-v1.0.json'),('Evidence Registry',REG['evidence']),('External Validation Registry',REG['external']),('Primitive Pressure Registry',REG['pressure']),('Adversarial Test Suite',REG['adversarial']),('Releases','docs/releases'),('Maintenance','docs/maintenance'),('Planning','docs/planning/README.md'),('Repository Index',INDEX)]
    lines += ['','## Repository Navigation','']+[f'- {link(p,label)}' for label,p in nav]
    lines += ['','## Current Roadmap','','- Current phase: P8 theorem-role decision','- Completed foundation for the active phase: architecture-neutral domain; IRD-001; PB-001; PBTS-001; deduction-first governance; THM-TARGET-001; premise ledger v1.1; FAITHFUL-REP-001','- In-progress work: select and justify coordinate, side_condition, or split for P8','- Planned work: S_core construction and obstruction ledger; formal negative-control lemmas; uniform constructor; finite-core theorem or refutation; extension analysis; lower bounds; minimality; mechanization; independent review','- Parallel supporting work: PBTS-001 independent replication, comparative evaluation, boundary discovery, and application validation','','## Command Center','','### Local Commands','','```bash\nmake research-check\nmake health-fast\nmake health\nmake docs-check\nmake plan\nmake dashboard\n```','','### GitHub Actions','',f'- Repository Health: {workflow("repository-health.yml")}',f'- Regenerate Dashboard: {workflow("regenerate-dashboard.yml")}',f'- Release Readiness: {workflow("release-readiness.yml")}',f'- Repository Maintenance: {workflow("repository-maintenance.yml")}','','## Typical Workflow','','1. `make research-check`','2. `make health-fast`','3. Open THM-TARGET-001 and FAITHFUL-REP-001','4. Resolve the highest unsatisfied theorem obligation','5. Freeze assumptions, failure conditions, and nonclaims','6. Prove, refute, or register the obstruction','7. Preserve failures and unresolved obligations','8. Run health','9. Merge','',END]
    return '\n'.join(lines)+'\n'
def main():
