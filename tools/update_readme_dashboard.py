...
def metrics():
    ...
    return ev,ex,adv,pressure,{
        'Internal reasoning systems':len(ev),
        'External reasoning systems':len(ex),
        'Adversarial fixtures':len(adv),
        'Counterexample fixtures':len(list((ROOT/'tests/counterexamples').glob('*'))) if (ROOT/'tests/counterexamples').exists() else len(list((ROOT/'tests').glob('*counter*'))) if (ROOT/'tests').exists() else 0,
        'Candidate primitive failures':failures,
        'Conservative extensions':cons,
        'Fits FAR':fits,
...
def generate_index():
    cats={
        ...
        'Evaluation':['theory/evaluation','docs/reports','tests'],
        'Reports':['reports','docs/reports'],
        'Releases':['docs/releases','docs/milestones'],
        'Planning':['docs/planning'],
        'Maintenance':['docs/maintenance','.github/workflows'],
...
def dashboard_generation_time():
    ...
    if history.exists():
        ...
        if isinstance(data,list) and data and isinstance(data[-1],dict) and data[-1].get('generated_at'):
            return data[-1]['generated_at']
    return datetime.now(timezone.utc).replace(microsecond=0).isoformat()

def block():
    generate_index()
    ev,ex,adv,pressure,m=metrics()
    gaps=gap_counts()
    rel=count_release()
    health='PASS'
    planner='CURRENT'
    ci='Push, pull-request, and manual workflows available'
    alerts=compute_alerts(health,ci)
    lines=[
        BEGIN,'',
        '## Repository Status','',
        f'- Current release: {link(rel) if rel else "unknown"}',
        '- Current project phase: W4 formal negative controls and W3.5 specificity/discovery in parallel',
        f'- Repository health status: {health} ({link("docs/maintenance/repository-health-checks.md","health checks")})',
        f'- Planner status: {planner} ({link("tools/self_advancement_plan.py","planner")})',
        f'- Last dashboard generation time: {dashboard_generation_time()}',
        '',
        '## Track Status','',
    ...
    src={
        'Internal reasoning systems':REG['evidence'],
        'External reasoning systems':REG['external'],
        'Adversarial fixtures':REG['adversarial'],
        'Counterexample fixtures':'tests',
        'Candidate primitive failures':REG['pressure'],
        'Conservative extensions':REG['pressure'],
        'Fits FAR':REG['evidence'],
        'Unresolved cases':STATUS,
    }
    for k,v in m.items():
        lines.append(f'| {k} | {v} | {link(src[k])} |')
    lines += [
        '',
    ...
    for tid,title,source,affected,branch,pr in parse_tasks():
        lines += [
            ...
            f'- Source: {source}',
            '- Affected files:',
            *[f'  - {a}' for a in affected],
            f'- Suggested branch: `{branch}`',
            f'- Suggested PR title: `{pr}`',
            ''
        ]
    lines += ['## Research Gap Summary','', '| Severity | Count | Source |','|---|---:|---|']
    for sev in ['Critical','High','Medium','Low']:
        lines.append(f'| {sev} | {gaps.get(sev,0)} | {link(GAPS)} |')
    nav=[
        ('Project Status',STATUS),
        ('Research Gap Report',GAPS),
        ('Next Actions',NEXT),
        ('Dashboard Metrics',METRICS),
        ('Representation–Discovery Separation','docs/governance/representation-discovery-separation-standard-v1.0.md'),
        ...
        ('Universal Candidate Registry','theory/evaluation/universal-structure-candidate-registry.json'),
        ...
        ('Evidence Registry',REG['evidence']),
        ('External Validation Registry',REG['external']),
        ('Primitive Pressure Registry',REG['pressure']),
        ('Adversarial Test Suite',REG['adversarial']),
    ...
    lines += [
        '',
        '## Current Roadmap','',
        '- REP: execute OBS-SC-010 and NC-01 through NC-10.',
        '- ADJ: execute W3.5 generic-baseline factorization, FARA-specificity, contrast, ablation, reconstruction, and claim impact.',
        '- USD: begin candidate-neutral scope execution after frozen admission controls are applied.',
        ...
        '3. Preserve countermodels, equivalences, failures, assumptions, and nonclaims.',
