...
import os, re, yaml
from report_link_utils import markdown_link, markdown_line_link, relative_href, slugify_heading
...
def heading_anchor(text): return '#'+slugify_heading(text)
def gap_counts():
    c=Counter()
    if GAPS.exists():
...
def workflow(name):
    ...
    return link(p, p.name) if p.exists() else f'`{name}` (not present)'
def generate_index():
    ...
    for cat,roots in cats.items():
        ...
        for r in roots:
            ...
            elif p.exists(): files += [x for x in p.rglob('*') if x.is_file() and '.git' not in x.parts]
...
def block():
    ...
    '## Typical Workflow','','1. `make health-fast`','2. `make dashboard`','3. Open README','4. Choose top task','5. Open linked files','6. Copy generated Codex prompt','7. Implement','8. Run health','9. Merge','',END]
    return '\n'.join(lines)+'\n'
def main():
