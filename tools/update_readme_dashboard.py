...
README=ROOT/'README.md'; OUT=README
...
def rewrite_links(markdown: str, source_file: Path, dest_file: Path) -> str:
    def repl(m):
        ...
        target=(source_file.parent/path).resolve(); new=relative_href(target,dest_file)+(('#'+frag) if frag else '')
        return f'[{label}]({new})'
    return re.sub(r'\[([^\]]+)\]\(([^)]+)\)',repl,markdown)
def parse_tasks(n=5):
    text=NEXT.read_text(encoding='utf-8') if NEXT.exists() else ''
    chunks=re.split(r'^### ((?:TASK|STRATEGIC)-\d+): (.+)$',text,flags=re.M); tasks=[]
...
def generate_index():
    ...
    lines=['# Repository Index','',f'Generated by {markdown_link("tools/update_readme_dashboard.py",INDEX)}. The root {markdown_link("README.md",INDEX,"README Command Center")} is the canonical entry point.','']; seen=set()
