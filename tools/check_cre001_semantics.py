#!/usr/bin/env python3
"""Validate frozen CRE-001 vocabulary semantics against sources and generated artifacts."""
...
NONCLAIMS=['universal sufficiency','primitive-only sufficiency','necessity','minimality','independence','superiority','FAR proof','universal reasoning structure']
...
def main()->int:
    ...
    for vocab,vdata in spec.get('vocabularies',{}).items():
        ...
        for prim in prims:
            primitive_records.append((vocab,prim))
    ...
    formal=doc_text.get('formal-semantics.md','')
    licensing=doc_text.get('semantic-licensing.md','')
    primitive_doc=doc_text.get('primitive-semantics.md','')
    for vocab,prim in primitive_records:
        name=prim['primitive_name']
        definition=prim['formal_definition']
    ...
    bad=[r'\bestablish(?:es|ed)? universal sufficiency\b',r'\bestablish(?:es|ed)? primitive-only sufficiency\b',r'\bestablish(?:es|ed)? necessity\b',r'\bestablish(?:es|ed)? minimality\b',r'\bestablish(?:es|ed)? independence\b',r'\bestablish(?:es|ed)? superiority\b',r'\bis a FAR proof\b']
