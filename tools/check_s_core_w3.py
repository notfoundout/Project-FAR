...
def main()->int:
    ...
    ledger=load(LEDGER); summary=ledger['execution_summary']; assert (summary['proved'],summary['obstruction_established'],summary['scope_boundary_established'],summary['refuted'],summary['open'])==(27,1,1,8,0); assert ledger['completed_waves']==['W0','W1','W2','W3','W4','W5']; assert ledger['active_wave']=='complete'; assert ledger['active_obligations']==[]
    by_id={x['id']:x for x in ledger['obligations']}; assert all(by_id[x]['status']=='proved' for x in PROVED); assert all(by_id[x]['status']=='refuted' for x in REFUTED); assert by_id['OBS-SC-010']['status']=='obstruction_established'; assert all(by_id[f'ASM-SC-{i:03d}']['status']=='proved' for i in range(1,4))
    target=load(TARGET); assert target['lemma_program']['proved_obligations']==27 and target['lemma_program']['open_obligations']==0; assert target['proof_status']=='bounded_theorem_proved'; assert target['next_required_artifact'].startswith('Mechanize the bounded W5 theorem'); assert target['w5_authorization']['authorized'] is True and target['w5_authorization']['blocked_by']==[]
    ...
    gates={x['name']:x for x in load(GATES)['gates']}; assert gates['formal-negative-controls']['status']=='satisfied'; assert gates['universal-structure-result']['status']=='not_satisfied'; assert gates['mechanized-proof-verification']['status']=='not_satisfied'; assert gates['independent-proof-review']['status']=='not_satisfied'
    ...
    print('S_core W3 proof: PASS (retained through W5)'); return 0
