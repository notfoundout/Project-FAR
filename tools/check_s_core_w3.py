#!/usr/bin/env python3
"""Validate SCORE-W3-PROOF-001 after later bounded W4 closure."""
...
def main()->int:
    ...
    gates={x['name']:x for x in load(GATES)['gates']}; assert gates['formal-negative-controls']['status']=='satisfied'; assert gates['scoped-representation-proof']['status']=='not_satisfied'; assert gates['baseline-factorization-resolved']['status']=='not_satisfied'; assert gates['mechanized-proof-verification']['status']=='not_satisfied'; assert gates['independent-proof-review']['status']=='not_satisfied'
    ...
    print('S_core W3 proof: PASS (W3 package unchanged; later W4 controls closed; W5 blocked by W3.5)'); return 0
