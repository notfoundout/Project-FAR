#!/usr/bin/env python3
"""Validate SCORE-W3-PROOF-001 and its conservative repository integration."""
...
DOC=ROOT/'docs/research/s-core-w3-global-witness-proof-v1.0.md'
REG=ROOT/'theory/evaluation/s-core-w3-global-witness-proof.json'
FIX=ROOT/'theory/evaluation/s-core-w3-reference-fixtures.json'
...
AUDIT=ROOT/'docs/audits/s-core-w3-proof-audit.md'
TEST=ROOT/'tests/test_s_core_w3_reference.py'
MAKE=ROOT/'Makefile'
PROVED={f'LEM-SC-{i:03d}' for i in range(17,25)}
REFUTED={'OBS-SC-002','OBS-SC-007','OBS-SC-008','OBS-SC-009'}

def load(p): return json.loads(p.read_text(encoding='utf-8'))

def main()->int:
    for p in (DOC,REG,FIX,LEDGER,TARGET,FAITHFUL,P8,GATES,AUDIT,TEST,MAKE): assert p.is_file(),p
    ...
    ledger=load(LEDGER); summary=ledger['execution_summary']; assert (summary['proved'],summary['scope_boundary_established'],summary['refuted'],summary['open'])==(24,1,8,4); assert ledger['completed_waves']==['W0','W1','W2','W3']; assert ledger['active_wave']=='W4'; assert ledger['active_obligations']==['OBS-SC-010']
    by_id={x['id']:x for x in ledger['obligations']}; assert all(by_id[x]['status']=='proved' for x in PROVED); assert all(by_id[x]['status']=='refuted' for x in REFUTED); assert by_id['OBS-SC-010']['status']=='registered_unproved'; assert all(by_id[f'ASM-SC-{i:03d}']['status']=='registered_unproved' for i in range(1,4))
    target=load(TARGET); assert target['lemma_program']['proved_obligations']==24 and target['lemma_program']['open_obligations']==4; assert target['proof_status']=='partial_lemma_progress_only'; assert target['next_required_artifact']==ledger['next_required_artifact']
    faithful=load(FAITHFUL); assert faithful['recovery_contract']['proof_status']=='proved_LEM-SC-018'; assert faithful['semantic_agreement']['global_proof_status']=='proved_LEM-SC-019'; assert faithful['nontriviality']['formal_negative_control_status']=='unproved_OBS-SC-010'
    ...
    print('S_core W3 proof: PASS (8 lemmas proved; 4 obstructions refuted; 4 obligations open; W4 active)')
    return 0
