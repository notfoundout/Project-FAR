...
def main()->int:
    ...
    target=load(TARGET); assert target['lemma_program']['proved_obligations']==24 and target['lemma_program']['open_obligations']==3; assert target['proof_status']=='partial_lemma_progress_only'; assert target['next_required_artifact'].startswith('W5 theorem assembly'); assert target['w5_authorization']['authorized'] is True and target['w5_authorization']['blocked_by']==[]
