...
def policy_errors(licensing,discrimination,specificity,w35,gates,target,candidates,factorization,digests):
    ...
    current=w35.get('current_results',{}); stage=w35.get('status'); require(stage in {'in_progress_specificity_complete','in_progress_candidate_complete'},'W3.5 status must preserve completed specificity'); require(w35.get('w5_authorized') is False,'W3.5 authorized W5'); require(current.get('reasoning_discrimination')=='bounded_role_conjunctive_discrimination_established','reasoning discrimination state changed'); require(current.get('fara_specificity')=='not_unique_at_registered_scope','FARA specificity state changed'); require(current.get('machinery_and_cost')=='not_executed','cost execution was started by specificity')
    artifacts={item.get('id'):item for item in w35.get('required_result_artifacts',[])}; scope=artifacts.get('W35-SCOPE-RESULT',{}); spec=artifacts.get('W35-SPEC-RESULT',{}); require(scope.get('status')=='complete' and scope.get('artifact_id')=='W35-SCOPE-RESULT-001' and scope.get('content_sha256')==digests.get('discrimination'),'W35 scope-result linkage invalid'); require(spec.get('status')=='complete' and spec.get('artifact_id')=='W35-SPEC-RESULT-001' and spec.get('content_sha256')==digests.get('specificity'),'W35 specificity-result linkage invalid')
    if stage=='in_progress_specificity_complete':
        require(current.get('candidate_invariants')=='not_executed','candidate execution was started by specificity'); require(artifacts.get('W35-CANDIDATE-RESULT',{}).get('status')=='missing','W35-CANDIDATE-RESULT was prematurely promoted'); require(all(item.get('current_classification')=='unresolved' for item in candidates.get('candidates',[])),'candidate classifications were changed'); require(candidates.get('aggregate_result')=='unresolved','candidate aggregate was changed')
    else:
        require(current.get('candidate_invariants')=='complete_no_indispensable_candidate','candidate execution state is inconsistent'); require(artifacts.get('W35-CANDIDATE-RESULT',{}).get('status')=='complete','candidate result is not complete'); require(all(item.get('current_classification')!='indispensable_within_frozen_class' for item in candidates.get('candidates',[])),'candidate classifications contain unsupported indispensability'); require(candidates.get('aggregate_result')=='no_registered_candidate_indispensable_within_frozen_class','candidate aggregate was changed')
