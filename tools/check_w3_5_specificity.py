...
def policy_errors(licensing,discrimination,specificity,w35,gates,target,candidates,factorization,digests):
    ...
    require(candidates.get('aggregate_result')=='unresolved_pending_evidence_complete_execution','candidate aggregate must remain unresolved'); require(candidates.get('status')=='candidate_execution_not_started','candidate registry status must remain unexecuted'); require(all(item.get('structural_commitment_necessity')=='unresolved' for item in candidates.get('candidates',[])),'structural necessity was prejudged')
