...
def policy_errors(licensing,discrimination,specificity,w35,gates,target,candidates,factorization,digests):
    ...
    current=w35.get('current_results',{}); stage=w35.get('status'); require(stage in {'in_progress_specificity_complete','in_progress_candidate_complete'},'W3.5 status is inconsistent with completed specificity'); require(w35.get('w5_authorized') is False,'W3.5 authorized W5'); require(current.get('reasoning_discrimination')=='bounded_role_conjunctive_discrimination_established','reasoning discrimination state changed'); require(current.get('fara_specificity')=='not_unique_at_registered_scope','FARA specificity state changed'); require(current.get('machinery_and_cost')=='not_executed','cross-package cost closure was prematurely promoted')
    expected_candidate='complete_registered_scope_internal_execution' if stage=='in_progress_candidate_complete' else 'not_executed'; require(current.get('candidate_invariants')==expected_candidate,'candidate execution stage is inconsistent')
    artifacts={item.get('id'):item for item in w35.get('required_result_artifacts',[])}; scope=artifacts.get('W35-SCOPE-RESULT',{}); spec=artifacts.get('W35-SPEC-RESULT',{}); candidate=artifacts.get('W35-CANDIDATE-RESULT',{}); require(scope.get('status')=='complete' and scope.get('artifact_id')=='W35-SCOPE-RESULT-001' and scope.get('content_sha256')==digests.get('discrimination'),'W35 scope-result linkage invalid'); require(spec.get('status')=='complete' and spec.get('artifact_id')=='W35-SPEC-RESULT-001' and spec.get('content_sha256')==digests.get('specificity'),'W35 specificity-result linkage invalid'); require(candidate.get('status')==('complete' if stage=='in_progress_candidate_complete' else 'missing'),'W35-CANDIDATE-RESULT stage is inconsistent')
    if stage=='in_progress_candidate_complete':
