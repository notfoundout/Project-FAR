...
def policy_errors(licensing,discrimination,specificity,w35,gates,target,candidates,factorization,digests):
    ...
    current=w35.get('current_results',{}); stage=w35.get('status'); require(stage in {'in_progress_specificity_complete','in_progress_candidate_complete','resolved'},'W3.5 status is inconsistent with completed specificity'); resolved=stage=='resolved'; require(w35.get('w5_authorized') is resolved,'W3.5 authorization stage is inconsistent'); require(current.get('reasoning_discrimination')=='bounded_role_conjunctive_discrimination_established','reasoning discrimination state changed'); require(current.get('fara_specificity')=='not_unique_at_registered_scope','FARA specificity state changed'); require(current.get('machinery_and_cost')==('complete' if resolved else 'not_executed'),'cross-package cost closure stage is inconsistent')
    expected_candidate='complete' if resolved else ('complete_registered_scope_internal_execution' if stage=='in_progress_candidate_complete' else 'not_executed'); require(current.get('candidate_invariants')==expected_candidate,'candidate execution stage is inconsistent')
    artifacts={item.get('id'):item for item in w35.get('required_result_artifacts',[])}; scope=artifacts.get('W35-SCOPE-RESULT',{}); spec=artifacts.get('W35-SPEC-RESULT',{}); candidate=artifacts.get('W35-CANDIDATE-RESULT',{}); require(scope.get('status')=='complete' and scope.get('artifact_id')=='W35-SCOPE-RESULT-001' and scope.get('content_sha256')==digests.get('discrimination'),'W35 scope-result linkage invalid'); require(spec.get('status')=='complete' and spec.get('artifact_id')=='W35-SPEC-RESULT-001' and spec.get('content_sha256')==digests.get('specificity'),'W35 specificity-result linkage invalid'); require(candidate.get('status')==('complete' if stage in {'in_progress_candidate_complete','resolved'} else 'missing'),'W35-CANDIDATE-RESULT stage is inconsistent')
    if stage in {'in_progress_candidate_complete','resolved'}:
        require(candidates.get('aggregate_result')=='registered_candidate_axes_resolved_at_frozen_internal_scope','candidate aggregate changed'); require(candidates.get('status')=='complete_project_authored_internal_execution','candidate registry completion status changed'); require(all(item.get('structural_commitment_necessity') in {'supported_at_registered_scope','refuted_at_registered_scope','partial'} for item in candidates.get('candidates',[])),'candidate structural axis is not terminal at registered scope'); require(all(item.get('trial_evidence_status')=='complete' for item in candidates.get('candidates',[])),'candidate trial evidence is incomplete')
    else:
        require(candidates.get('aggregate_result')=='candidate_structural_indispensability_unresolved_reexecution_required','candidate aggregate must remain unresolved before execution'); require(candidates.get('status')=='preliminary_internal_adjudication_reexecution_required','candidate registry status must remain preliminary before execution')
    for artifact_id in ('W35-COST-RESULT','W35-CLAIM-RESULT','W35-FAILURE-RESULT'): require(artifacts.get(artifact_id,{}).get('status')==('complete' if resolved else 'missing'),f'{artifact_id} closure stage is inconsistent')
    ...
    auth=target.get('w5_authorization',{}); require(auth.get('authorized') is resolved,'THM-TARGET-001 authorization stage is inconsistent'); require(auth.get('blocked_by')==([] if resolved else ['W3.5-SDG-001']),'THM-TARGET-001 blocker stage is inconsistent')
    return errors
...
def validate(root:Path=ROOT)->dict[str,Any]:
    ...
    return {'status':'pass','experiment_id':runtime['experiment_id'],'instance_count':runtime['scope']['instances'],'class_counts':runtime['class_counts'],'fara_specificity':runtime['specificity_analysis']['FARA_specificity_result'],'w5_authorized':static['w35']['w5_authorized']}
def main(argv:list[str]|None=None)->int:
    ...
    else: print(f"W3.5 specificity validation: PASS (8 positive; 8 contrast; 2 disputed preserved; FARA not unique at registered scope; W5 authorized={report['w5_authorized']})")
    return 0
