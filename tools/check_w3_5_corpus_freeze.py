...
REQ_OBS={
'observable_or_formal_states','transitions_and_constraints','represented_distinctions','declared_semantics',
'history_and_path_dependence','objectives_or_tasks','grounds_or_dependencies','commitment_or_alternative_structure',
'uncertainty_and_revision','provenance_and_correspondence_limits'}
BANNED=('fara','grel','universal structure','candidate invariant','pb-001','expected w3.5')
PATHS={
'scope':'theory/evaluation/reasoning-and-contrast-scope-v1.0.json',
'catalog':'theory/evaluation/rcs-concrete-source-catalog-v1.0.json',
'positive':'theory/evaluation/rcs-positive-corpus-v1.0.json',
'contrast':'theory/evaluation/rcs-contrast-corpus-v1.0.json',
'disputed':'theory/evaluation/rcs-disputed-corpus-v1.0.json',
'result':'theory/evaluation/w3-5-corpus-freeze-result-v1.0.json',
'w35':'theory/evaluation/w3-5-specificity-and-discovery-gate.json',
'gates':'theory/evaluation/research-gates.json',
'doc':'docs/research/w3-5-concrete-corpus-freeze-v1.0.md',
'audit':'docs/audits/w3-5-concrete-corpus-freeze-audit.md'}
...
def validate_data(*,scope,catalog,source_records,registries,result,w35,gates,actual_digests,actual_bundle_digests)->list[str]:
    ...
    req(scope.get('execution_status')=='ready_for_candidate_neutral_execution','corpus must be ready but not executed',e)
    ...
    for r in records:
        ...
        if isinstance(c,dict):
            for key in ('kind','source_type','authoritative_fields','observation_boundary','excluded_processes','replay_requirement'):
                req(bool(c.get(key)),f'{iid} contract lacks {key}',e)
    ...
    mins={'positive':8,'contrast':8,'disputed':2}
    reg_ids={'positive':'RCS-POSITIVE-CORPUS-001','contrast':'RCS-CONTRAST-CORPUS-001','disputed':'RCS-DISPUTED-CORPUS-001'}
    reg_instance_ids={}
    ...
    for key in ('all_decisions_precede_candidate_scoring','all_candidate_exposure_statuses_explicit','candidate_registry_preexisted','candidate_scores_or_results_not_exposed_before_admission','all_records_have_source_or_observation_contracts','all_records_have_formalization_boundaries','all_records_have_candidate_neutral_observations','all_records_replayable_within_boundary'):
        req(result.get('admission_assurance',{}).get(key) is True,f'freeze result assurance {key} must be true',e)
    for result_key,digest_key in {'source_catalog':'catalog','positive_registry':'positive','contrast_registry':'contrast','disputed_registry':'disputed'}.items():
        req(result.get('artifacts',{}).get(result_key,{}).get('content_sha256')==actual_digests.get(digest_key),f'freeze result digest mismatch for {result_key}',e)

    req(w35.get('status')=='in_progress_corpus_frozen','W3.5 must remain in progress after corpus freeze',e)
    ...
    for aid,a in arts.items():
        if aid!='W35-CORPUS-RESULT': req(a.get('status')=='missing',f'{aid} must remain missing',e)
    ...
    for name in ('baseline-factorization-resolved','fara-specificity-resolved','reasoning-contrast-execution','universal-structure-result'):
        req(gm.get(name,{}).get('status')=='not_satisfied',f'{name} must remain not_satisfied',e)
    return e
...
def validate(root:Path=ROOT)->list[str]:
    ...
    catalog=load(root/PATHS['catalog'])
    source_records=[]; bundle_digests={}; bundles={}
    for entry in catalog.get('records',[]):
        ...
        if relative not in bundles:
            bundles[relative]=load(path); bundle_digests[relative]=sha(path)
        records=bundles[relative].get('records',[])
...
def main()->int:
    ...
    if errors:
        ...
        return 1
    print('W3.5 concrete corpus freeze PASS (8 positive; 8 contrast; 2 disputed; candidate scoring not started; W5 blocked)')
    return 0
