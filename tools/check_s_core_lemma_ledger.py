#!/usr/bin/env python3
"""Validate the frozen S_core lemma ledger and its W0 execution state."""
...
def main() -> int:
    for path in (DOC, REG, TARGET, GATES, W0_DOC, W0_REG, MAKEFILE):
    ...
    for phrase in (
        ...
        "No target-construction lemma",
        "Construct or obstruct the W1 base-carrier and direct-axis package",
    ):
    ...
    assert data.get("status") == "frozen_dependency_decomposition_w0_complete_w1_active"
    ...
    assert data.get("completed_waves") == ["W0"]
    assert data.get("active_wave") == "W1"
    ...
    for lemma_id in ("LEM-SC-001", "LEM-SC-002", "LEM-SC-003", "LEM-SC-004"):
        item = by_id[lemma_id]
        assert item.get("status") == "proved"
        assert item.get("evidence") == [W0_DOC.relative_to(ROOT).as_posix(), W0_REG.relative_to(ROOT).as_posix()]
    assert by_id["OBS-SC-001"].get("status") == "scope_boundary_established"
    assert by_id["OBS-SC-001"].get("evidence") == [W0_DOC.relative_to(ROOT).as_posix(), W0_REG.relative_to(ROOT).as_posix()]
    ...
    expected_active = {"LEM-SC-005", "LEM-SC-006", "LEM-SC-007", "LEM-SC-008", "LEM-SC-009", "LEM-SC-012", "LEM-SC-014"}
    assert active == expected_active
    assert all(by_id[item_id]["wave"] == "W1" for item_id in active)
    ...
    assert status_counts["proved"] == summary["proved"]
    assert status_counts["scope_boundary_established"] == summary["scope_boundary_established"]
    ...
    for path in (DOC, REG, W0_DOC, W0_REG):
    ...
    print("S_core lemma ledger: PASS (W0 complete; 4 proved; 1 boundary; 32 open; W1 active)")
    return 0
