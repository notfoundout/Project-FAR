#!/usr/bin/env python3
"""Validate deduction-first dependencies after bounded W4 closure."""
...
def main()->int:
    ...
    names={x['name']:x for x in load(F['gates'])['gates']}; assert names['formal-negative-controls']['status']=='satisfied'; assert names['scoped-representation-proof']['status']=='not_satisfied'; assert names['baseline-factorization-resolved']['status']=='not_satisfied'; assert names['mechanized-proof-verification']['status']=='not_satisfied'; assert names['independent-proof-review']['status']=='not_satisfied'
    ...
    print('Deduction-first research program: PASS (W0-W4 complete; evidence-backed W3.5 blocks W5; theorem gate closed)'); return 0
