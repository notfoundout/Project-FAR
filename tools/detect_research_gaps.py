...
def add(gaps, typ, loc, sev, action):
    gaps.append({'type':typ,'location':loc,'severity':sev,'action':action})
...
def main():
    ...
    reports=set(str(p.relative_to(ROOT)) for p in (ROOT/'theory/evaluation/external-systems').glob('*.md'))
    ...
    for p in [ROOT/'README.md', ROOT/'docs/reports/project-far-v0.3.0-synthesis.md', ROOT/'docs/project-status.md']:
        if p.exists() and 'latest' in p.read_text(encoding='utf-8').lower() and re.search(r'v0\.[0-3]\.', p.read_text(encoding='utf-8')):
            release_refs.append(str(p.relative_to(ROOT)))
    for r in release_refs: add(gaps,'stale latest-release reference',r,'high','Review release wording against current release baseline.')
    ...
    for base in REPORT_DIRS:
        for p in base.rglob('*.md'):
            ...
            if pat.search(text): add(gaps,'TODO/future-work note',rel,'low','Review note and decide whether it blocks validation.')
    ...
    for i,g in enumerate(gaps,1): lines.append(f"| GAP-{i:03d} | {g['type']} | `{g['location']}` | {g['severity']} | {g['action']} |")
