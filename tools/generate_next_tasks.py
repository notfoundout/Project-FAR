...
def main():
    ...
    lines=['# Next Actions','', f'Generated by `python tools/generate_next_tasks.py` from {markdown_link(STATUS, OUT)} and {markdown_link(GAP, OUT)}. These recommendations are advisory only and do not authorize theory changes.','', '## Ranked Next Actions','']
    for t in tasks:
