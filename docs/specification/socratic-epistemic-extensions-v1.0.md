...
- `explicit_nonclaims`;
- basis-bearing `falsifiers`;
- basis-bearing `surviving_propositions`;
- basis-bearing `residual_uncertainty`;
- basis-bearing `limitations`;
- basis-bearing `assumptions`;
- `closure_status`;
...
- derived implications with explicit premise references, calculus, rule, implication context, and any required per-premise context bridges;
...
5. derive implications only through an explicitly named calculus or inferential rule and record the implication context;
6. compare implications and commitments only at compatible contexts; any premise whose recorded context differs from the implication context requires an explicit bridge tied to that premise;
...
A context bridge records an explicit justification for transporting one named premise into the implication context. It does not by itself establish that the transport is substantively valid. A bridge for a non-premise or multiple bridges for one premise is invalid.
...
7. Cross-record source binding must be checked against resolved source revisions rather than inferred from duplicated IDs or scopes.
8. Neither layer establishes factual correctness, domain adequacy, source truth, semantic completeness, or external validation.
...
- Stage 6 may use elicited commitments as premises only at their recorded scope and context. A different-context premise requires an explicit per-premise bridge into the declared implication context.
- Evidence appraisal may use expertise applicability as one typed input only after exact source-revision binding has been checked; it never substitutes for direct evidence or repairs an invalid inference.
...
- `frameworks/FAR/dependency-graph.md` must register the expertise and elenchus supporting-protocol dependencies whenever these accepted documents remain part of FAR.
...
- missing exact expertise-assertion or claim version fields in an applicability record;
- resolver-supplied expertise assertion or claim revisions whose IDs, versions, or scopes do not match the applicability record;
- one identical scoped boundary statement placed in incompatible terminal categories;
- a falsifier, surviving proposition, residual-uncertainty item, limitation, or assumption lacking item-level basis references;
...
- an implication whose premise context differs from the declared implication context without a per-premise bridge;
- an implication context bridge that references a non-premise or duplicates a bridge for one premise;
...
Natural-language expertise, truth, entailment, contradiction, completeness, and the substantive validity of a context bridge are outside the validator's decidable scope. Lifecycle chronology establishes only internally possible recorded ordering; it does not prove wall-clock accuracy or external causality.
...
- binds applicability only to stable IDs while leaving the referenced expertise-assertion or claim revision ambiguous;
- reuses an applicability result after a referenced source revision changes without re-evaluation;
- treats expertise as proof of the source's substantive claim;
- fabricates an applicability bridge;
- stores a falsifier, surviving proposition, residual uncertainty, limitation, or assumption without item-level basis provenance;
...
- derives across materially different commitment contexts without an explicit premise-specific bridge;
- supplies a context bridge for a commitment that is not actually a premise;
...
The research record provides the elimination/reduction analysis. The implementation is tested by positive fixtures, adversarial mutations, resolver-assisted binding regressions, and separately coded internal reference oracles for the central expertise-scope, boundary-exclusivity/closure-linkage, elenchus-provenance/history, context-compatibility, and lifecycle-chronology invariants.
