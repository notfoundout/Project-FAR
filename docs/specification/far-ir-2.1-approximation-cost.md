...
**Stage 2 — reference, metric, loss, cost dimensions.** Appended in this order: `DUPLICATE_REFERENCE_CASE`, `REFERENCE_COVERAGE_MISMATCH`, `REFERENCE_NOT_PROBABILITY`, `DUPLICATE_METRIC_VALUE`, `DUPLICATE_METRIC_ENTRY`, then either `METRIC_NOT_TOTAL` — which suppresses all four metric-axiom checks — or the metric-axiom checks, interleaved rather than grouped by code: for each distinct declared metric value `l` in first-occurrence order, `METRIC_IDENTITY_FAILURE` if `d(l, l) != 0`; then for each distinct value `r` in the same order, `METRIC_SYMMETRY_FAILURE` if `d(l, r) != d(r, l)` and `METRIC_SEPARATION_FAILURE` if `l != r` and `d(l, r) == 0`, followed, for each distinct value `m` in the same order, by `METRIC_TRIANGLE_FAILURE` if `d(l, r) > d(l, m) + d(m, r)` (so an asymmetric pair is reported once from each side); then `DUPLICATE_LOSS_ACTION`, `DUPLICATE_LOSS_ENTRY`, `LOSS_NOT_TOTAL`, then for each distinct required-behavior value in first-occurrence order of `required_behavior.table` rows and, within it, each distinct declared action in first-occurrence order, `METRIC_LOSS_DOMAIN_MISMATCH` if that (truth, action) pair is not a metric pair, otherwise `LOSS_METRIC_MISMATCH` if its loss entry differs from the metric distance; and finally `DUPLICATE_COST_DIMENSION`. Values are identified by the canonical JSON of `far-ir/2.0` §5.

The first-occurrence order of the loss checks was fixed after W5 execution: the executed verifier iterated required-behavior values as a Python set, so the relative order of those two codes varied with `PYTHONHASHSEED`. Its executed bytes are preserved at [`research/results/pca-w5-approximation-and-cost/frozen-inputs/contract_v21.py`](../../research/results/pca-w5-approximation-and-cost/frozen-inputs/contract_v21.py), from which W5 is recomputed.
