...
**Established defect:** `status` is non-univocal across canonical FARA. *(Corrected the same day by the `OP-18` term audit below. The five-sense premise is **refuted**; `status` is univocal. The surviving defect is narrower: `admissibility-structure.md:111` leaves the categorization of `unresolved status` unstated. The smallest repair remains a scope clarification stating whether the explicit unresolved result falls within "admissibility classification"; which interpretation to select is underdetermined and is an `ADR-002` decision point. The `H5` verdict and the path-linking and unknown-cause results of this entry do not depend on the withdrawn premise and stand.)* The repair site is identified as a single scope clarification, but **the repair content is not supplied**, because the direction of clarification is exactly the undetermined question. Recorded as `LIM-030`/`OP-18`/`UQ-T22`.
...
## 2026-08-11 — Freeze the R5 cross-context replication protocol

**Question:** Can the dominant evidence deficit recorded by `LIM-031` — that no party outside the programme has ever chosen or evaluated a preservation contract for the universality question — be made testable by an externally executable protocol?
**Execution:** Constructed a fourteen-artifact protocol package at `theory/evaluation/r5-cross-context/`. Froze the exact participant wording of a primary packet and a structurally different mutation packet; a twelve-item contamination questionnaire with six levels; a Role A output schema in which `unsettled` is a valid value; a three-role separation with an ordered freeze-before-reveal procedure; a normalization schema requiring both mapping directions and preserving failures; an adjudication procedure privileging the respondent's own comparison criterion; a twelve-outcome registry with per-outcome evidence requirements; five evidence tiers each with an explicit `cannot_establish` list; a provenance manifest; an adversarial package audit; and a verifier enforcing blinding, structure, and state.
**Observation:** Two defects were found and repaired during construction. The first draft of the question described the phenomenon in terms that gestured at three target requirements at once, and was replaced with a formulation asserting no structure about what happens between question and conclusion. The verifier initially passed **vacuously**, because it cut the scanned span at the first non-delivered heading, which in both packets is section 1; the delivered span is now extracted explicitly with a character floor that fails closed. A subsequent substring match on short acronyms fired inside ordinary words, which was repaired by word-boundary matching, on the ground that a check that cries wolf trains its reader to ignore it. The verifier was then confirmed live by a negative control against a deliberately leaked packet.
**Discovery/acceptance:** The package is accepted as **frozen protocol only**. Its state is `PROTOCOL FROZEN / EVIDENCE NOT YET COLLECTED`. One residual limitation is disclosed rather than dismissed: the A–I decomposition in the primary packet is itself a structural hint about which axes are worth separating, retained because removing it would make outputs unmappable without the mapper supplying structure, and mitigated by the mutation control. *(Superseded 2026-08-11: the original mutation packet reproduced the A–I axes and added a substantive pragmatic criterion; it is replaced by B1 semantic-preserving reword and B2 decomposition-ablation. See the R5 protocol repair entry below.)*
**Repository change:** New protocol directory plus two register notes recording the protocol's existence and frozen status. **No R5 result, independent recovery, replication success, architecture support, or contract support is registered.** `LIM-031` is unchanged and remains open. `OP-21`, `UQ-T25`, `UQ-T26`, `G2`, `IKD-W9`, `TUE-W4`, `USD-W6`, `FAR-CANONICAL-UNIVERSALITY-DECISION-001`, `ADR-002`, and every canonical surface are unaffected. `ADR-002` remains unselected. Nothing was executed.
