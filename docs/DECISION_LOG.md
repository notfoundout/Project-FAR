...
**Discovery/acceptance:** The package is accepted as **frozen protocol only**. Its state is `PROTOCOL FROZEN / EVIDENCE NOT YET COLLECTED`. One residual limitation is disclosed rather than dismissed: the A–I decomposition in the primary packet is itself a structural hint about which axes are worth separating, retained because removing it would make outputs unmappable without the mapper supplying structure, and mitigated by the mutation packet, which asks the same question without that decomposition.
