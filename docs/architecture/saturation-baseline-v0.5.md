...
1. Determine whether v0.5 can represent it without changing a first-class primitive, invariant, assurance boundary, or extension interface.
...
| Layer | First-class capabilities required by v0.5 |
|---|---|
...
For finite, explicit Project FAR implementations, the Accepted FARA kernel remains canonical. The architecture may expose a typed, versioned epistemic graph as an operational representation or derived view only when its mapping to the FARA identity-bearing many-sorted relational structure preserves the registered commitments, carrier distinctions, occurrence identities, Event/State/Rule separations, and admission constraints required by current repository authority. It is **not** globally required to be a DAG. Only derivation subgraphs that require acyclicity should enforce it.
...
These dimensions require independent typed assessments rather than a shared claim/evidence enum.
...
Minimum evidence-set sufficiency/appraisal vocabulary should separately support, where applicable:
...
Individual evidence-item quality, relevance, directness, independence, and applicability remain separately typed dimensions and are not claim truth statuses.
...
Every audit that performs evidence retrieval must preserve the search process, not only the citations ultimately selected.
...
ACQUIRED_AT       = when FAR observed, retrieved, or otherwise obtained the information
TRANSACTION_TIME  = when the information was committed to FAR's authoritative record
```
...
ECOSYSTEM
  ...
  +--> MUTATION EVENTS
  +--> EXPOSURE EVENTS
  +--> COORDINATION / DEPENDENCE ASSERTIONS
  +--> INTERVENTIONS
  +--> OBSERVED CONSEQUENCES
```

Ecosystem claims remain evidence-bearing and challengeable. The system must distinguish observed propagation from inferred exposure, coordination, influence, or intervention effect. It must not infer actor intent from propagation structure alone.

## Governance-obligation assurance

Truth adjudication and obligation satisfaction are orthogonal. A true claim can be handled in violation of an applicable obligation; procedural compliance does not establish substantive truth.

A governance obligation must be able to preserve:
...
subject / system
required or prohibited property
formal or machine-testable specification where available
evidence requirement
satisfaction test
effective time / version
challenges / interpretation disputes
```

Minimum assessment states are:
...
source lineage
status date / transaction-time/acquisition-time boundary
```
...
security scanner
challenge workflow
ecosystem model / propagation validator
obligation adapter / applicability validator / compliance checker
synthesis renderer
