...
Frozen proof-dependency ledger for `THM-TARGET-001`; W0, W1, and W2 complete; W3 active.
...
The ledger records 16 proved construction lemmas, 1 established source-scope boundary, 4 refuted obstruction hypotheses, and 16 open obligations. It does not establish admissible target-only recovery, any complete `Pres_i` predicate, `Faithful_split`, FARA adequacy, or a scoped representation theorem.
...
W1 and W2 prove target structures and strong embeddings for their registered finite reducts. The frozen `Pres_i` definitions also require admissible target-only recovery, semantic agreement, coherence, and the other W3 obligations. Therefore no complete `Pres_i` predicate is reported as proved.
...
- `LEM-SC-022` — uniformity and source-isomorphism equivariance — **registered unproved**.
...
- `OBS-SC-002`: determine whether some in-scope source necessarily requires a hidden interpreter, source oracle, or case database.
...
- `OBS-SC-007`: determine whether successful witnesses necessarily require nonuniform case branching or an unbounded helper family.
- `OBS-SC-008`: determine whether a source-declared composition necessarily loses interface or cross-component structure.
- `OBS-SC-009`: determine whether the frozen target interface is insufficient after complete witness assembly.
...
| Proved construction obligations | 16 |
...
Execute the W3 global-witness package as one dependency-closed construction or obstruction program. Recovery must be target-only and deterministic; semantics and identity must agree across axes; every helper and metadata field must be counted; the constructor must be uniform and equivariant; declared decompositions must compose; and the final witness must be well typed.
