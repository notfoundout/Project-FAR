...
Completed predecessor: [`POST-CLOSURE-001`](governance/post-closure-assurance-and-application-program-v1.0.md), complete at its six registered workstream scopes.

Current program: [`EXTERNAL-FALSIFICATION-AND-REPLICATION-001`](governance/external-falsification-and-replication-program-v1.0.md), preregistered and not executed. It is not W7. The [canonical W1–W6 matrix](governance/w1-w6-claim-evidence-matrix-v1.0.md) fixes the evidence inherited by the program.

The core theory remains closed after the v1.1 correction. The historical UPP queue remains closed, and its defective frozen derivation is not a target for silent repair. The hostile audit that triggered v1.1 remains non-independent; the later sealed W1 campaign returned 14 `PROVED` verdicts under I1 claimed isolation, not I3 external independent validation, and did not establish novelty or priority. W2 mechanized all 14 governed claims under the pinned Lean toolchain without finding a contradiction. W3 established the versioned `far-ir/2.0` comparison-contract schema and finite-explicit semantic verifier without changing `far-ir/1.0`. W4 froze six native-domain contracts before mapping and produced six checked lossy collisions plus six checked repaired factorizations at bounded finite scopes. W5 added bounded `far-ir/2.1` approximation and multidimensional cost semantics. W6 then preregistered and executed a bounded internal controlled-artifact audit test without promoting that result to human or external utility.
...
W6 used the six W4 repaired records as registered negative controls and deterministic collision mutants as positive cases. The schema-only lane was a syntactic machine baseline, not unaided human review. The separate table-only oracle provides implementation-diverse internal replication of the collision predicate, not external independent validation.

## Gates

Every workstream must preserve scope, provenance, typed outcomes, negative results, `Unknown`, and claim impact. CI success, model agreement, finite panels, successful encoding, and the non-independent correction audit are not mathematical proof or independent validation. The promoted W1 record changes independent-review assurance; Lean changes formalization assurance; W3 conformance changes only encoded contract/schema assurance; W6 changes only bounded internal controlled-artifact audit evidence.
...
Apply the contract-relative theory accurately: expose consequential choices, certify factorization or collisions, compute or approximate contract-indexed quotients, report losses and `Unknown`, test utility under separately governed empirical protocols, and retract any claim that exceeds its contract.
