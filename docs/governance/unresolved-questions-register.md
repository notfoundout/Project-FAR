...
- **UQ-T10A:** Are the `FARA-VOC-001` extension candidates necessary outside its frozen auxiliary model? **Unresolved; finite mappings and ablations cannot establish global necessity or minimality.**
- **UQ-T11:** Which coherent formal foundation, identity criteria, and equivalence relation match intended canonical FARA without substantive theory change? **Resolved for Project FAR v1.0 repository authority by `FARA-CANONICAL-KERNEL-001`: the identity-bearing many-sorted relational kernel is canonical within the finite explicit auditable scope. Typed-hypergraph and algebraic/state-transition structures remain derived views. Global uniqueness, necessity, minimality, completeness, and applicability beyond scope remain unresolved.**
- **UQ-T12:** Would independent candidate implementations, neutral reformulations, or executable oracle/nonfinite/environment semantics change the Pareto relation among the retained historical FARA foundations? **Unresolved; `FARA-FOUNDATION-COMP-001` finds many-sorted relational → algebraic/state-transition under the frozen dimensions, while typed hypergraph remains incomparable with many-sorted relational. The later canonical-kernel decision does not rewrite that bounded comparison.**
...
- **UQ-T14:** Does the canonical identity-bearing many-sorted relational kernel remain adequate for nonfinite continuous, embodied, distributed, changing-ontology, or live-oracle systems? **Unresolved and explicitly outside the scoped preregistered experiment. This limits the experiment's conclusions but does not block execution over the frozen finite explicit source-system class.**
