...
- **UQ-T22:** In which sense is `status` used in each canonical FARA occurrence, and does "unresolved status" denote an admissibility status? **Unresolved; five distinct senses are used without disambiguation, and no canonical text asserts that unresolved status is an admissibility status. See `LIM-030`, `OP-18`.**
