...
The H1/A1 machine lane never uses a custodian's label or supplied report as its prediction. Preserve the complete sealed exact contract, replace any input report uniformly with the candidate `PROVED` factorization/first-source-case decoder construction specified in the U1 projection procedure, recompute its contract freeze, and seal this derived record before the first FAR run. Run the exact baseline `contract_v2` JSON command. A well-formed `success: true` means accepted preservation; `success: false` means flagged loss. Missing/malformed output or a runtime failure is an item failure, not a correct detection or an exclusion. Retain the original and derived record hashes, all diagnostics, and transformation map. The same prepared H1 records/reports supply HD1; no new case-dependent projection or report choice is permitted for the human study. R2 separately retains the original registered conformance inputs and their declared report semantics.

For H1, custodians use only the source frame below, screen pre-existing external records in ascending public creation timestamp, breaking ties by source identifier, and take the first ten eligible records per domain and class. Every screened record and exclusion is retained. Eligibility requires an independently sourced real domain problem, a finite-explicit behavior/representation table, and a determinate label; copied or transformed W4/W6 fixtures are excluded. Two independent domain judges label before any FAR run; a third resolves differences by a recorded majority with a checkable witness. Unresolved labels are excluded before sealing, never after outcomes are seen. FAR's exact baseline implementation is the tested system, with no tuning on H1 or A1.

#### Frozen H1 source frame

H1's source universe is the bounded arXiv v1 literature frame produced by this exact discovery procedure, not an operator-selected collection or an estimate of all external domain cases. Query the arXiv Atom API at `https://export.arxiv.org/api/query` with each expression in the table, conjoined with `submittedDate:[199101010000 TO 202609042359]`. URL-encode the complete expression. Set `sortBy=submittedDate`, `sortOrder=ascending`, `max_results=100`, and `start=0,100,200,...` until every reported result is retrieved; space requests by at least three seconds. Archive every request, response, retrieval timestamp, total count, and SHA-256 before screening. A result set over 30,000 entries, inconsistent pagination/counts, or inaccessible required response invalidates intake; no query refinement or source substitution is permitted.

| Domain ID | Exact search expression before the date conjunction |
|---|---|
| `argumentation` | `all:"argumentation"` |
| `bayesian_causal` | `(all:"Bayesian" OR all:"causal reasoning")` |
| `formal_logic` | `all:"formal logic"` |
| `model_based_reasoning` | `all:"model based reasoning"` |
| `proof_theory` | `all:"proof theory"` |
| `type_theory` | `all:"type theory"` |

Within each complete response set, deduplicate versionless arXiv IDs, sort by v1 `published` timestamp then versionless ID lexically, and retain the first 1,000 papers (or all if fewer). Use the public `v1` PDF only; do not use a later revision, citation expansion, other repository, or custodian-supplied replacement. The resulting six sets and all v1 PDF hashes form the source-frame seal. A paper present in multiple sets belongs only to the first matching domain in the table's order; do not refill a set after this assignment. A missing, withdrawn, unreadable, or unhashable selected v1 PDF invalidates that domain stratum rather than silently shrinking or replacing the frame. Later index changes cannot replace the archived intake snapshot.

Read each assigned paper in PDF page order and top-to-bottom reading order (left column before right); screen its explicitly worked finite examples in that order and take at most its first eligible example. The source must already state the finite source cases, representation relation, and required behavior sufficiently to transcribe the complete table without inventing values, a counterfactual defect, or a repair. Custodian transcription and domain/label adjudication must cite exact source pages and retain all decisions; ambiguous extraction is ineligible. A paper's abstract alone, a general theorem without a worked finite record, or a newly constructed illustration is ineligible. Order selected examples by their source's v1 timestamp and ID, then apply the first-ten-per-class rule. Failure to obtain all twenty cases in any domain is `INVALID/INCONCLUSIVE`; the search frame, count, and thresholds cannot be enlarged or relaxed to rescue it. The frame has not been screened during this preregistration, so its ability to supply the required cases is unestablished.
...
Enroll the first three consenting independent organizations with an existing routine documentary audit workflow, at least 40 forthcoming investigations, lawful access to the evidence, and an applicable ethics determination. Before counting an organization as an eligible site, inspect and seal its existing SOP: every enrolled investigation must be performable by one auditor acting alone. SOPs requiring peer review, dual control, consultation, team deliberation, or another person's assistance are ineligible; no mandatory role may be removed to make a site eligible. The existing SOP must also be executable within the registered English-language documentary interface and sixty-minute task limit without changing its substantive steps. Record all site-screening decisions before enrollment. Eligible investigations audit an existing finite record and a declared required behavior from one of the six domains; exclude consequential medical/legal/financial decisions, live interventions, missing consent, and open-oracle cases. Record exclusions prospectively. At each site enroll the next 40 eligible investigations within 180 days, with no outcome-based replacement. Failure to enroll all 40 makes that site's study inconclusive.

Before generating any U1 arm allocation, seal each site's worker roster. Eligible workers are consenting adults aged at least 18 who already performed that site's routine documentary audits under its existing SOP before site enrollment, are outside Project FAR control and artifact authorship, and pass the same frozen English-language course, twelve-record classification/witness qualification, and environment conformance rules as HD1. Invite all such existing auditors; take the first at most forty who consent and qualify within thirty calendar days after site consent, in qualification-completion order with the pre-consent sequential screening-log entry breaking exact ties. Retain invitations, refusals, failed attempts, and credentials establishing their existing audit role. A site with no qualifying worker is inconclusive. The roster closes at day thirty; no later recruitment, substitution, or reranking is allowed.

Assign worker IDs S01-W001, S01-W002, etc. in that sealed order (and analogously for S02/S03). For a site with N workers, investigation enrollment index i=1..40 is assigned to worker position `(i-1) mod N`, using zero-based positions in the sealed roster. Seal all forty worker-to-investigation-index assignments with the site roster before its arm-allocation seed is derived or any slot is revealed. Assignment cannot depend on arm, case difficulty, availability after reveal, or outcomes. The assigned worker alone performs that investigation; no task swapping or assistance is allowed. Worker absence or withdrawal leaves the assigned output missing under the original intention-to-treat and cost/burden rules, with no replacement. Case enrollment and its assigned worker are append-only sealed before the corresponding arm is revealed.

Before enrollment seal each site's existing standard procedure verbatim; both arms receive the same native evidence and sealed SOP, along with the common instructions and outcome/workload capture fields specified in the materials. The FAR arm additionally receives the exact W3 `far-ir/2.0` contract fields and that format's baseline verifier report; the U1 standard arm receives neither of these additions. This differs from HD1, where the finite contract tables are common evidence and only the report is arm-dependent. W5 `far-ir/2.1` approximation, tolerance, and cost declarations are not intervention fields in U1. Training is the same four-hour course as HD1. Audit time limit is 60 minutes in both arms. For each site shuffle 20 standard and 20 FAR slots using the sealed roster-manifest seed; reveal only the next slot after its case is enrolled and hashed. Site weights are equal. An escaped material defect is a behavior-changing representation error that the final submitted audit fails to identify with a correct witness. Three external adjudicators, blinded to arm and aggregate rates, inspect source evidence and determine this binary endpoint by a recorded majority; all reasons and dissent remain public in deidentified form.

Reduction is standard minus FAR defect proportion; relative reduction divides that difference by the standard proportion. A zero standard proportion cannot pass the benefit gate. Use 10,000 crossed resamples of whole workers within each site and investigations within each site/arm, under the exact loop below, for the equally site-weighted reduction. Each site's unrounded observed reduction must be at least −0.05. Assess harms through 30 days after each audit. Serious harm means death, hospitalization, permanent impairment, unauthorized sensitive-data disclosure, or an irreversible consequential action attributable to reliance on the FAR-assisted audit; any confirmed event or harm-related safety stop fails the safety gate. No such action is an intended part of this study.

Every U1 projection follows the separately frozen [deterministic projection procedure](../research/external-falsification-and-replication/02-u1-projection.md). Independent custodians use the pre-enrollment SOP field map, agree on all native rows, construct the fixed exact contract and uniform candidate report, and seal both the complete projection and baseline diagnostic report for every investigation before its arm is revealed. Non-unique native extraction is ineligible before enrollment; adverse verifier outputs cannot be excluded or repaired after execution.
...
Attribute enrolled-person training/setup/practice costs equally to that person's allocated tasks, with the task's actual arm receiving its share; HD1 has sixty tasks per enrolled person. For U1, use each worker's site and investigation-index assignments sealed before arm allocation under the cyclic rule above; charge their training only to those investigations at that site, never another site. Site-specific recruitment, failed-candidate training, and common overhead are divided equally over that site's forty allocated investigations. Only central U1 recruitment/overhead that cannot be attributed to a person or site is split in exact thirds across S01–S03 before division by forty; record it separately. HD1's non-person-attributable recruitment, failed-candidate training, and common overhead are divided equally across its 1,440 ratings. Charge directly attributable task expenditure to that task. These categories are mutually exclusive, every ledger entry is charged once, and there is no pooled redistribution of site-specific or participant-specific costs across U1 sites. Publish total burden including unsuccessful recruitment even when the arm difference cancels a common charge.
...
FAR-specific preparation is a distinct cost category: source-field mapping, contract transcription/projection and its adjudication, candidate-record construction, and verifier/report production. For U1, charge all such preparation costs within a site equally to its twenty FAR investigations, including the hidden preparation done for standard cases to preserve blinding. Do not charge them to standard or common overhead and thereby cancel FAR's burden. For HD1, the common native/contract evidence is shared, but report-generation costs for each case are charged to that case's six FAR ratings only. Record activity categories before arm reveal and count each expense once; the complete ledger also reports the extra research/blinding burden. This conservative study-cost allocation is not an estimate of commercial deployment cost.
...
All rates, differences, percentile sorting, and primary threshold comparisons use exact rational arithmetic from integer counts and bootstrap multiplicities (Python `fractions.Fraction`), not rounded floating-point values. Convert to decimal only for display after the verdict. There are no random calls other than those specified here and in the frozen HD1 allocator.

HD1 uses the content-addressed [analysis script](../../tools/efr_hd1_analysis.py), which verifies the sealed input-manifest and ratings digests and reconstructs the frozen allocation before calculating endpoints. The ratings object contains `input_manifest_sha256` and exactly 1,440 `ratings` rows, each naming `reviewer_id`, `case_id`, `arm`, and `decision` (`preservation`, `material_loss`, or JSON null for missing). Every allocated cell needs a row; duplicate, unallocated, relabelled-manifest, or omitted cells fail closed. Other recorded fields remain in the sealed ledger but do not change the primary calculation. Run under Python 3.12 as `python tools/efr_hd1_analysis.py INPUT INPUT_SHA256 RATINGS RATINGS_SHA256`; both expected hashes come from their signed independent deposits.

The observed endpoints retain equal case weights and unit reviewer weights, exactly reproducing the registered 15-pair-per-case disagreement and 360-loss-rating-per-arm false-accept proportions. For resampling, start a fresh `random.Random(hd1_seed)` once. The loop schedule is fixed:

```python
for resample in range(10000):
    case_weights = Counter()
    for domain in DOMAINS:
        for label in CLASSES:
            stratum = sorted_ids[domain, label]
            for draw in range(10):
                case_weights[stratum[rng.randrange(10)]] += 1
    # One shared multiplicity for each original reviewer across every task/arm.
    reviewer_weights = Counter(sorted_reviewer_ids[rng.randrange(24)] for draw in range(24))
    # Compute BOTH endpoints with these same case and reviewer weights; no extra draws.
```

For each original unordered pair of distinct reviewers r,s on case c, weight its disagreement indicator by `case_weights[c] * reviewer_weights[r] * reviewer_weights[s]`. Divide the sum of these weighted indicators by the sum of the pair weights separately in each arm; reduction is standard minus FAR. Do not form a fictitious pair between two copies of the same original person. For the loss-case safety endpoint, weight each original rating by `case_weights[c] * reviewer_weights[r]`, divide wrong-answer weight by total loss-rating weight in each arm, and subtract standard from FAR. Apply the frozen missingness imputations before weighting. Thus repeated reviewer behavior changes all of that person's case contributions together instead of being held fixed. If any required denominator is zero, record `INVALID_UNESTIMABLE_RESAMPLE` for the test; do not discard or redraw it.

Sort the 10,000 exact fractions independently for each endpoint and select indices 249 and 9749. Acceptance requires observed reduction ≥`Fraction(1,10)`, its lower percentile >0, and the false-accept difference's upper percentile ≤`Fraction(1,50)`, plus all validity gates. The script reports numerical gates only; independent blinding, ethics, provenance, and protocol-validity findings remain mandatory. This crossed perturbation interval is a preregistered robustness gate with nominal 95% percentile bounds. It does not assert exact frequentist coverage, population representativeness, or a theorem that the crossed-effects results cited below extend unchanged to this sparse dyadic disagreement statistic. Report both reviewer and case dependence and the finite-cohort scope with every result.

U1 site IDs are S01–S03 in eligible consent-timestamp order, breaking exact ties by legal organization name. Investigation IDs are the site ID plus the three-digit consecutive enrollment index. The complete site-roster manifest binds identities, SOP hashes, qualified worker rosters, all forty predetermined worker-to-investigation-index assignments per site, and allocation inputs; its independently sealed SHA-256 supplies `u1_seed`. Allocation starts a fresh `random.Random(u1_seed)`, iterates sites S01, S02, S03, and for each starts `slots = ["far"] * 20 + ["standard"] * 20`, calls `rng.shuffle(slots)` exactly once, and seals that site's slots. There are no intervening random calls; reveal slots only after prospective case enrollment and its assigned-worker seal.

U1 analysis uses a separate fresh stream with the same seed. For each site/arm sort the twenty allocated investigation IDs lexically, including missing outputs as escaped defects. For each site, sort its complete sealed worker roster lexically. Draw whole-worker multiplicities shared across both arms and all their investigations, then draw investigations. The loop is normative:
...
for resample in range(10000):
    ...
    for site in ("S01", "S02", "S03"):
        workers = sorted_worker_ids[site]
        n = len(workers)
        weights = Counter(workers[rng.randrange(n)] for draw in range(n))
        for arm in ("far", "standard"):
            numerator = denominator = 0
            for draw in range(20):
                case_id = sorted_ids[site, arm][rng.randrange(20)]
                weight = weights[assigned_worker[case_id]]
                numerator += weight * escaped_defect[case_id]
                denominator += weight
            # Zero denominator invalidates the test; no discarded/redrawn resample.
            rates[site, arm] = Fraction(numerator, denominator)
    reduction = sum(rates[s, "standard"] - rates[s, "far"]
                    for s in ("S01", "S02", "S03")) / 3
    # Store the exact fraction; no other random calls.
```

Use indices 249 and 9749 of sorted resample reductions. Observed reduction uses each original investigation once and equal site weights. Its absolute gate is ≥`Fraction(1,20)`; relative reduction is that observed difference divided by the observed standard-arm defect rate and must be ≥`Fraction(1,5)` (zero denominator fails). The lower percentile must exceed zero, each observed site reduction must be ≥`-Fraction(1,20)`, and the separately observed harm gate must pass. As for HD1, this nominal crossed perturbation interval is a robustness gate, not an exact-coverage or population confidence claim. In particular three selected sites, and a site with only one worker, cannot supply uncertainty about an unrestricted population of sites or workers. The loops preserve repeated worker contributions and fix computation; they do not authorize early stopping, refreezing labels, or changing the recorded sites/cases/workers.
...
- Owen and Eckles, [Bootstrapping data arrays of arbitrary order](https://arxiv.org/abs/1106.2125) — motivation for resampling crossed dependence factors; its sufficient-condition results are not claimed as a coverage theorem for HD1.

- [arXiv API user manual](https://info.arxiv.org/help/api/user-manual.html) — query syntax, pagination, submission-date ordering, and version metadata for the frozen H1 discovery procedure; the frame and selection limits above are this program's choices.
