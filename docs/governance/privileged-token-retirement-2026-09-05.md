...
Protected PR #472 authorized retirement of only that captured value. The inline pin and governed receipt matched; a different value failed before API calls. This is evidence about the observed repository credential, not an invented assertion about an earlier unobserved token. Retirement was still pending at that activation stage.
