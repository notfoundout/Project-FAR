...
| `PCA-W5-APPROXIMATION-AND-COST` finite controls | **PROVED at the frozen finite-explicit operational scope** | Exact executable checks establish only the registered approximation, Pareto/least-element, and zero-loss-boundary results; this is not a universal cost or optimization theorem. |
| `PCA-W6-EMPIRICAL-AUDIT-UTILITY` controlled artifact campaign | **PROVED for the registered corpus and collision mutation; empirical, not a core theorem** | The semantic audit detected 6/6 mutants with 0/6 false positives on clean controls and agreed with the independent table-only oracle on 12/12 primary items. Human and external effectiveness remain untested. |
...
The hostile audit that triggered v1.1 remains **non-independent**. The separately sealed W1 review is complete and confirms truth under exact scopes; it does not establish novelty or priority. W2 is a separate assurance dimension: all 14 governed claims are formalized and no contradiction was found. W4 adds bounded application certificates under six frozen contracts; W5 adds bounded operational controls; W6 adds bounded internal artifact-level empirical evidence. None of W4–W6 changes the core theorem statuses. See the generated [assurance ledger](../assurance/far-core-assurance.md) for per-claim dimensions.
