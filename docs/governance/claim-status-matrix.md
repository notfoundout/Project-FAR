...
Evidence: `docs/research/contract-frontier-discovery-v1.0.md`. Investigation target `OP-21`/`UQ-T25`/`UQ-T26`. No canonical surface was modified. No registered claim is upgraded or downgraded.

| Claim | Status | Required interpretation |
|---|---|---|
| An independently motivated external contract reproduces or dominates `C*`/`P*`/`E*` | **Not established** | `H1` and `H3` rejected. No external contract in the searched space reproduces the conjunction, and none dominates it — each trades a comparison dimension against it. |
| `C*`/`P*`/`E*` is independently justified | **Not established — `H5`** | Every `P*` component is independently anticipated; the *conjunction* is not recovered by any framework located. This replicates the `OP-20` componentwise-anticipation pattern one architectural layer up. |
| `C*`/`P*`/`E*` occupies a defensible Pareto-frontier position | **`H2`, restricted** | Established only relative to seven comparison dimensions selected with knowledge of that contract's profile. **Weaker than the `H5` finding**, which does not depend on the dimension set. Never cite `H2` as uniqueness or as vindication. |
| A coverage-monotone total order over contracts is admissible | **Refuted** | Three of seven dimensions — preservation strength, recovery strength, observational discrimination — are anti-monotone with source coverage. The coverage-maximal contract is the degenerate bare-set contract, whose forced architecture is trivial. Only a Pareto relation is admissible. |
| Serious independently motivated contracts are Pareto-comparable | **Refuted** | Blackwell, testing, institutional, and minimal-realization contracts are pairwise incomparable maximal elements. `C*`/`P*`/`E*` is incomparable with each. |
| No non-trivial comparison among contracts is justified | **Rejected — `H6` refuted** | Three independently developed comparison orders exist and were applied at their own scopes: van Glabbeek's discriminating-power order within LTS processes, the Blackwell order within information structures over a common state space, and the precision order within abstract domains over a fixed concrete semantics. |
| **RCCD is contract-invariant** | **Refuted — RCCD is contract-relative** | `G2` derives RCCD from `P*`/`E*`. Six independently motivated contracts force materially different canonical objects: minimal sufficient statistic, final coalgebra, fully abstract model, institution, best abstraction, Nerode automaton. Only `P*`/`E*` forces RCCD. This states `G2`'s reach exactly as `G2` states it; **no downgrade follows**. |
| **FARA is contract-invariant** | **Refuted — contract-relative and realization-plural** | FARA is one realization inside `CEC-RCCD-001` (`IKD-W8`), and that class is itself relative to `P*`/`E*`. `OP-20` independently established componentwise anticipation of the kernel with the connecting architecture unrecovered. |
| Every contract forces the same architecture | **Refuted** | No two surviving contracts force the same canonical object. |
| The surviving contracts force *incompatible* architectures | **Rejected as an overreading** | Each forces the canonical quotient of its own declared observations. The invariant across contracts is the **construction pattern**, not the architecture. Independently established in five separate mature fields, and confirming the surviving positive residue of the 2026-08-05 target-category-discovery audit. |
...
| Project FAR's failure to independently justify its contract is a project defect | **Rejected** | Contract selection is a known unsolved problem in at least three mature independent fields. Concurrency theory has not converged on which observations are "reasonable"; the Blackwell order is deliberately partial and most experiments are unrankable; universal coalgebra is parametric in the observation functor by design and its own literature records that with no observation component every system becomes equivalent. |
| `FAR-CANONICAL-UNIVERSALITY-DECISION-001`'s `not_derivable` is a gap awaiting closure | **Rejected as an interpretation; the verdict itself is unchanged** | Results conditioned on a declared contract and presented over a frontier are the **normal terminal form** for this class of question in every adjacent field. `ADR-002` is the same phenomenon at the intra-FARA scale: where evidence underdetermines, the residue is a decision, not a discoverable fact. |
| This campaign constitutes external independence | **Rejected** | The external sources are independent; the reconstruction, dimension selection, scoring, and placement are not. Single path, no mutation controls, no external reviewer. **Weaker than `USD-W6`'s `internal_robustness_only`.** Recorded as `LIM-031`. |
