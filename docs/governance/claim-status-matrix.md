...
| `unresolved status` denotes an `admissibility status` | Unresolved | The lexical bridge from `:23` would establish it, but no text asserts the identification. **Basis corrected 2026-08-10:** this row originally cited "five distinct senses" of `status`, which the `OP-18` audit **refuted** — see the `status` term audit below and `LIM-030`. The surviving basis is narrower and still sufficient: the compound `N status` is systematically ambiguous between naming the categorization and naming the value, and `admissibility-structure.md:111` is the sole technical occurrence with no governing definition and no stated categorization. The verdict is unchanged. |
