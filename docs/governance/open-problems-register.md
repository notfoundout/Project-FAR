...
| OP-13 | Determine whether faithful representation of set-valued admissibility requires retyping `Ω`, a conservative extension, or only semantic clarification. | theoretical | **Resolved at finite scope in favour of a conservative extension.** Every function `C → Status` is refuted by an equivariance pigeonhole argument; an extension-indexed family of unchanged canonical `Ω` structures is faithful and conservative over existing primitives. Retyping is not required for set-valued admissibility. Infinite extension families and graded acceptability remain outside the result. Evidence: `docs/research/fara-omega-set-valued-admissibility-v1.0.md`. |
| OP-14 | Determine whether graded or ranking-based acceptability semantics require retyping `Ω`'s `Status` codomain. | theoretical | The `OP-13` equivariance argument suggests any finite `Status` set is vulnerable to the same pigeonhole pressure when a source admits more equivariance classes than `Status` has values. Not adjudicated. |
